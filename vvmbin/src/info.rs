...
pub struct TxInput<'a, T> {
	...
	pub fork_spec_name: &'a vapjson::spec::ForkSpec,
	...
	pub env_info: &'a tetsy_vm::EnvInfo,
...
pub fn run_transaction<T: Informant>(
	...
	let fork_spec = match VvmTestClient::fork_spec_from_json(&fork_spec_name) {
		Some(spec) => {
		...
		match result {
			Ok(TransactSuccess { state_root, gas_left, output, vm_trace, end_state, .. }) => {
				if state_root != post_root {
					(Err(VvmTestError::PostCondition(format!(
...
pub fn run<'a, F, X>(
	...
	F: FnOnce(VvmTestClient) -> (Result<Vec<u8>, VvmTestError>, H256, Option<PodState>, Option<U256>, Option<X>),
{
	let do_dump = trie_spec == TrieSpec::Fat;

	let mut test_client = VvmTestClient::from_pod_state_with_trie(spec, pre_state.clone(), trie_spec)
