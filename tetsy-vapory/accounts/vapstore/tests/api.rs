...
use vapstore::{VapStore, SimpleSecretStore, SecretVaultRef, StoreAccountRef};
use tetsy_crypto::publickey::{Random, Generator, Secret, KeyPair, verify_address};
...
fn secret_store_create() {
	...
	let _ = VapStore::open(Box::new(dir)).unwrap();
}
...
fn secret_store_create_account() {
	let dir = TransientDir::create().unwrap();
	let store = VapStore::open(Box::new(dir)).unwrap();
...
fn test_path() -> &'static str {
	match ::std::fs::metadata("vapstore") {
		Ok(_) => "vapstore/tests/res/gvap_keystore",
 		Err(_) => "tests/res/gvap_keystore",
	}
}

fn pat_path() -> &'static str {
	match ::std::fs::metadata("vapstore") {
		Ok(_) => "vapstore/tests/res/pat",
		Err(_) => "tests/res/pat",
	}
}
...
fn ciphertext_path() -> &'static str {
	match ::std::fs::metadata("vapstore") {
		Ok(_) => "vapstore/tests/res/ciphertext",
		Err(_) => "tests/res/ciphertext",
	}
}

#[test]
fn secret_store_laod_gvap_files() {
