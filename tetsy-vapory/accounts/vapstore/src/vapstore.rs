...
impl VapStore {
	...
	pub fn open_with_iterations(directory: Box<dyn KeyDirectory>, iterations: u32) -> Result<Self, Error> {
		Ok(VapStore {
			store: VapMultiStore::open_with_iterations(directory, iterations)?,
		})
	}
...
impl SimpleSecretStore for VapStore {
...
impl SecretStore for VapStore {
	...
	fn list_gvap_accounts(&self, testnet: bool) -> Vec<Address> {
		import::read_gvap_accounts(testnet)
	}

	fn import_gvap_accounts(&self, vault: SecretVaultRef, desired: Vec<Address>, testnet: bool) -> Result<Vec<StoreAccountRef>, Error> {
		let imported_addresses = match vault {
			SecretVaultRef::Root => import::import_gvap_accounts(&*self.store.dir, desired.into_iter().collect(), testnet),
			SecretVaultRef::Vault(vault_name) => {
				if let Some(vault) = self.store.vaults.lock().get(&vault_name) {
					import::import_gvap_accounts(vault.as_key_directory(), desired.into_iter().collect(), testnet)
				} else {
...
mod tests {
	...
	use super::{VapStore, VapMultiStore};
	...
	fn store() -> VapStore {
		VapStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory always load successfuly; qed")
	}

	fn multi_store() -> VapMultiStore {
		VapMultiStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory always load successfuly; qed")
	}
	...
	fn should_create_and_open_vaults() {
		...
		let store = VapStore::open(dir.key_dir.take().unwrap()).unwrap();
