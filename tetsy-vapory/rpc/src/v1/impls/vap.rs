...
use tetsy_rlp::Rlp;
use vapory_types::{Address, H64, H160, H256, U64, U256, BigEndianHash};
...
use v1::types::{
	...
	Transaction, CallRequest, Index, Filter, Log, Receipt, Work, VapAccount, StorageProof,
	block_number_to_id
};
...
impl VapClientOptions {
	/// Creates new default `VapClientOptions` and allows alterations
...
impl Default for VapClientOptions {
	fn default() -> Self {
		VapClientOptions {
...
/// Vap rpc implementation.
pub struct VapClient<C, SN: ?Sized, S: ?Sized, M, EM> where
...
pub fn base_logs<C, M, T: StateInfo + 'static> (client: &C, miner: &M, filter: Filter) -> BoxFuture<Vec<Log>> where
	...
	let filter: VapcoreFilter = match filter.try_into() {
		Ok(value) => value,
...
impl<C, SN: ?Sized, S: ?Sized, M, EM, T> VapClient<C, SN, S, M, EM> where
...
pub fn pending_logs<M>(miner: &M, best_block: VapBlockNumber, filter: &VapcoreFilter) -> Vec<Log> where M: MinerService {
...
impl<C, SN: ?Sized, S: ?Sized, M, EM, T: StateInfo + 'static> Vap for VapClient<C, SN, S, M, EM> where
	...
	EM: ExternalMinerService + 'static,
{
	...
	fn proof(&self, address: H160, values: Vec<H256>, num: Option<BlockNumber>) -> BoxFuture<VapAccount> {
		...
		let res = match self.client.prove_account(key1, id) {
			Some((proof, account)) => Ok(VapAccount {
				address,
