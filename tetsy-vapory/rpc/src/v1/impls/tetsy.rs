...
//! Tetsy-specific rpc implementation.
use std::sync::Arc;
...
impl<C, M, U> TetsyClient<C, M, U> where
	C: BlockChainClient,
{
	/// Creates new `TetsyClient`.
	pub fn new(
		client: Arc<C>,
		miner: Arc<M>,
		sync: Arc<dyn SyncProvider>,
...
impl<C, M, U, S> Tetsy for TetsyClient<C, M, U> where
	S: StateInfo + 'static,
	...
	U: UpdateService + 'static,
{
	...
	fn future_transactions(&self) -> Result<Vec<Transaction>> {
		Err(errors::deprecated("Use `tetsy_allTransaction` instead."))
	}
