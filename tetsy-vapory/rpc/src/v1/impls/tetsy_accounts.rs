...
use vapory_types::{Address, H160, H256, H520};
use vapkey::{Brain, Password};
...
/// Account management (personal) rpc implementation.
pub struct TetsyAccountsClient {
	accounts: Arc<AccountProvider>,
	deprecation_notice: DeprecationNotice,
}

impl TetsyAccountsClient {
...
impl TetsyAccountsInfo for TetsyAccountsClient {
	fn accounts_info(&self) -> Result<BTreeMap<H160, AccountInfo>> {
		self.deprecation_notice("tetsy_accountsInfo");
	...
	fn default_account(&self) -> Result<H160> {
		self.deprecation_notice("tetsy_defaultAccount");
...
impl TetsyAccounts for TetsyAccountsClient {
	...
	fn new_account_from_phrase(&self, phrase: String, pass: Password) -> Result<H160> {
		self.deprecation_notice("tetsy_newAccountFromPhrase");
	...
	fn new_account_from_wallet(&self, json: String, pass: Password) -> Result<H160> {
		self.deprecation_notice("tetsy_newAccountFromWallet");
	...
	fn new_account_from_secret(&self, secret: H256, pass: Password) -> Result<H160> {
		self.deprecation_notice("tetsy_newAccountFromSecret");
	...
	fn test_password(&self, account: H160, password: Password) -> Result<bool> {
		self.deprecation_notice("tetsy_testPassword");
	...
	fn change_password(&self, account: H160, password: Password, new_password: Password) -> Result<bool> {
		self.deprecation_notice("tetsy_changePassword");
		let account: Address = account.into();
		self.accounts
	...
	fn kill_account(&self, account: H160, password: Password) -> Result<bool> {
		self.deprecation_notice("tetsy_killAccount");
		let account: Address = account.into();
		self.accounts
	...
	fn remove_address(&self, addr: H160) -> Result<bool> {
		self.deprecation_notice("tetsy_removeAddresss");
	...
	fn set_account_name(&self, addr: H160, name: String) -> Result<bool> {
		self.deprecation_notice("tetsy_setAccountName");
		let addr: Address = addr.into();
	...
	fn set_account_meta(&self, addr: H160, meta: String) -> Result<bool> {
		self.deprecation_notice("tetsy_setAccountMeta");
		let addr: Address = addr.into();
	...
	fn import_gvap_accounts(&self, addresses: Vec<H160>) -> Result<Vec<H160>> {
		self.deprecation_notice("tetsy_importGvapAccounts");
		self.accounts
			.import_gvap_accounts(into_vec(addresses), false)
			.map(into_vec)
			.map_err(|e| errors::account("Couldn't import Gvap accounts", e))
	}

	fn gvap_accounts(&self) -> Result<Vec<H160>> {
		self.deprecation_notice("tetsy_listGvapAccounts");
		Ok(into_vec(self.accounts.list_gvap_accounts(false)))
	}

	fn create_vault(&self, name: String, password: Password) -> Result<bool> {
		self.deprecation_notice("tetsy_newVault");
	...
	fn open_vault(&self, name: String, password: Password) -> Result<bool> {
		self.deprecation_notice("tetsy_openVault");
	...
	fn close_vault(&self, name: String) -> Result<bool> {
		self.deprecation_notice("tetsy_closeVault");
	...
	fn list_vaults(&self) -> Result<Vec<String>> {
		self.deprecation_notice("tetsy_listVaults");
	...
	fn list_opened_vaults(&self) -> Result<Vec<String>> {
		self.deprecation_notice("tetsy_listOpenedVaults");
	...
	fn change_vault_password(&self, name: String, new_password: Password) -> Result<bool> {
		self.deprecation_notice("tetsy_changeVaultPassword");
	...
	fn change_vault(&self, address: H160, new_vault: String) -> Result<bool> {
		self.deprecation_notice("tetsy_changeVault");
		self.accounts
	...
	fn get_vault_meta(&self, name: String) -> Result<String> {
		self.deprecation_notice("tetsy_getVaultMeta");
	...
	fn set_vault_meta(&self, name: String, meta: String) -> Result<bool> {
		self.deprecation_notice("tetsy_setVaultMeta");
	...
	fn derive_key_index(&self, addr: H160, password: Password, derivation: DeriveHierarchical, save_as_account: bool) -> Result<H160> {
		self.deprecation_notice("tetsy_deriveAddressIndex");
		let addr: Address = addr.into();
		self.accounts
			.derive_account(
				&addr,
	...
	fn derive_key_hash(&self, addr: H160, password: Password, derivation: DeriveHash, save_as_account: bool) -> Result<H160> {
		self.deprecation_notice("tetsy_deriveAddressHash");
		let addr: Address = addr.into();
		self.accounts
			.derive_account(
				&addr,
	...
	fn export_account(&self, addr: H160, password: Password) -> Result<KeyFile> {
		self.deprecation_notice("tetsy_exportAccount");
		let addr = addr.into();
		self.accounts
			.export_account(
				&addr,
	...
	fn sign_message(&self, addr: H160, password: Password, message: H256) -> Result<H520> {
		self.deprecation_notice("tetsy_signMessage");
		self.accounts
			.sign(
				addr.into(),
				Some(password),
