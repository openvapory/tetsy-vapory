...
impl<D: Dispatcher + 'static> TetsySigning for SigningQueueClient<D> {
	...
	fn post_sign(&self, meta: Metadata, address: H160, data: RpcBytes) -> BoxFuture<RpcEither<U256, RpcConfirmationResponse>> {
		self.deprecation_notice.print("tetsy_postSign", deprecated::msgs::ACCOUNTS);
		let executor = self.executor.clone();
		let confirmations = self.confirmations.clone();

		Box::new(self.dispatch(
			RpcConfirmationPayload::VapSignMessage((address, data).into()),
			meta.origin
	...
	fn post_transaction(&self, meta: Metadata, request: RpcTransactionRequest) -> BoxFuture<RpcEither<U256, RpcConfirmationResponse>> {
		self.deprecation_notice.print("tetsy_postTransaction", deprecated::msgs::ACCOUNTS);
		let executor = self.executor.clone();
		let confirmations = self.confirmations.clone();
	...
	fn check_request(&self, id: U256) -> Result<Option<RpcConfirmationResponse>> {
		self.deprecation_notice.print("tetsy_checkRequest", deprecated::msgs::ACCOUNTS);
...
impl<D: Dispatcher + 'static> VapSigning for SigningQueueClient<D> {
	type Metadata = Metadata;
