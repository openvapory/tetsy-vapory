...
use std::path::Path;
use vapcore::client::{ClientConfig, DatabaseCompactionProfile};
use super::tetsy_kvdb_rocksdb::{CompactionProfile, DatabaseConfig};
...
pub fn memory_per_column(total: Option<usize>) -> HashMap<u32, usize> {
	...
	if let Some(budget) = total {
		...
		memory_per_column.insert(vapcore_db::COL_STATE, std::cmp::max(budget * 9 / 10, 256));
		// spread the remaining 10% evenly across columns
		let rest_budget = budget / 10 / (vapcore_db::NUM_COLUMNS as usize - 1);

		for i in 1..vapcore_db::NUM_COLUMNS {
...
pub fn memory_per_column_light(total: usize) -> HashMap<u32, usize> {
	...
	let per_column = total / (vapcore_db::NUM_COLUMNS as usize - 1);
...
pub fn client_db_config(client_path: &Path, client_config: &ClientConfig) -> DatabaseConfig {
	let mut client_db_config = DatabaseConfig::with_columns(vapcore_db::NUM_COLUMNS);
