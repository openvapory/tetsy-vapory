...
pub use tetsy_rpc::signer::SignerService;
...
use tetsy_rpc::dispatch::{FullDispatcher, LightDispatcher};
use tetsy_rpc::informant::{ActivityNotifier, ClientNotifier};
use tetsy_rpc::{Host, Metadata, NetworkSettings};
use tetsy_rpc::v1::traits::TransactionsPool;
...
pub enum Api {
	...
	/// Vap Pub-Sub (Safe)
	VapPubSub,
	/// Gvap-compatible "personal" API (DEPRECATED; only used in `--gvap` mode.)
	Personal,
...
impl FromStr for Api {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		...
		match s {
			...
			"net" => Ok(Net),
			"tetsy" => Ok(Tetsy),
			"tetsy_accounts" => Ok(TetsyAccounts),
			"tetsy_pubsub" => Ok(TetsyPubSub),
			"tetsy_set" => Ok(TetsySet),
			...
			"traces" => Ok(Traces),
			"web3" => Ok(Web3),
			"tetsy_transactions_pool" => Ok(TetsyTransactionsPool),
...
fn to_modules(apis: &HashSet<Api>) -> BTreeMap<String, String> {
	...
	for api in apis {
		let (name, version) = match *api {
			Api::Debug => ("debug", "1.0"),
			Api::Vap => ("vap", "1.0"),
			Api::VapPubSub => ("pubsub", "1.0"),
			Api::Net => ("net", "1.0"),
			Api::Tetsy => ("tetsy", "1.0"),
			Api::TetsyAccounts => ("tetsy_accounts", "1.0"),
			Api::TetsyPubSub => ("tetsy_pubsub", "1.0"),
			Api::TetsySet => ("tetsy_set", "1.0"),
			...
			Api::Web3 => ("web3", "1.0"),
			Api::TetsyTransactionsPool => ("tetsy_transactions_pool", "1.0"),
			Api::Deprecated => {
				continue;
			}
		};
			...
			match *api {
				...
				Api::Vap => {
					let client = VapClient::new(
						...
						&self.external_miner,
						VapClientOptions {
							pending_nonce_from_queue: self.gvap_compatibility,
							allow_pending_receipt_query: !self.gvap_compatibility,
							send_block_number_in_get_work: !self.gvap_compatibility,
					...
					if !for_generic_pubsub {
						let filter_client = VapFilterClient::new(
							self.client.clone(),
							self.miner.clone(),
							self.poll_lifetime,
						);
						...
						add_signing_methods!(VapSigning, handler, self, (&dispatcher, &account_signer));
					}
				}
				Api::VapPubSub => {
					if !for_generic_pubsub {
						...
						let mut client =
							VapPubSubClient::new(self.client.clone(), self.executor.clone(), pool_receiver);
				...
				Api::Tetsy => {
					...
					#[cfg(feature = "accounts")]
					handler.extend_with(TetsyAccountsInfo::to_delegate(TetsyAccountsClient::new(&self.accounts)));

					if !for_generic_pubsub {
						add_signing_methods!(TetsySigning, handler, self, (&dispatcher, &account_signer));
					}
				}
				Api::TetsyPubSub => {
					if !for_generic_pubsub {
				...
				Api::TetsyAccounts => {
					#[cfg(feature = "accounts")]
					handler.extend_with(TetsyAccounts::to_delegate(TetsyAccountsClient::new(&self.accounts)));
				}
				Api::TetsySet => {
					handler.extend_with(
						TetsySetClient::new(
	...
	fn extend_api<T: core::Middleware<Metadata>>(
		...
		for api in apis {
			match *api {
				...
				Api::Vap => {
					let client = light::VapClient::new(
						self.sync.clone(),
						self.client.clone(),
						self.on_demand.clone(),
						self.transaction_queue.clone(),
					...
					handler.extend_with(Vap::to_delegate(client.clone()));

					if !for_generic_pubsub {
						handler.extend_with(VapFilter::to_delegate(client));
						add_signing_methods!(VapSigning, handler, self, (&dispatcher, &account_signer));
					}
				}
				Api::VapPubSub => {
					...
					let mut client = VapPubSubClient::light(
						self.client.clone(),
						self.on_demand.clone(),
						self.sync.clone(),
						self.cache.clone(),
					...
					handler.extend_with(VapPubSub::to_delegate(client));
				}
				Api::TetsyTransactionsPool => {
					if !for_generic_pubsub {
				...
				Api::Tetsy => {
					...
					#[cfg(feature = "accounts")]
					handler.extend_with(
						TetsyAccountsInfo::to_delegate(TetsyAccountsClient::new(&self.accounts))
					);
				...
				Api::TetsySet => handler.extend_with(
					light::TetsySetClient::new(self.client.clone(), self.sync.clone(), self.fetch.clone())
						.to_delegate(),
				),
	...
	pub fn list_apis(&self) -> HashSet<Api> {
		...
		match *self {
			...
			ApiSet::UnsafeContext => {
				...
				public_list.insert(Api::TetsyPubSub);
				public_list.insert(Api::TetsyTransactionsPool);
				public_list
			}
			ApiSet::IpcContext => {
				public_list.insert(Api::Traces);
				public_list.insert(Api::TetsyPubSub);
				public_list.insert(Api::TetsyAccounts);
				public_list.insert(Api::TetsyTransactionsPool);
				public_list
			}
			ApiSet::All => {
				...
				public_list.insert(Api::TetsyPubSub);
				public_list.insert(Api::TetsyAccounts);
				public_list.insert(Api::TetsySet);
...
mod test {
	...
	fn test_api_parsing() {
		...
		assert_eq!(Api::Vap, "vap".parse().unwrap());
		assert_eq!(Api::VapPubSub, "pubsub".parse().unwrap());
		assert_eq!(Api::Personal, "personal".parse().unwrap());
		assert_eq!(Api::Signer, "signer".parse().unwrap());
		assert_eq!(Api::Tetsy, "tetsy".parse().unwrap());
		assert_eq!(Api::TetsyAccounts, "tetsy_accounts".parse().unwrap());
		assert_eq!(Api::TetsySet, "tetsy_set".parse().unwrap());
		assert_eq!(Api::Traces, "traces".parse().unwrap());
		assert_eq!(Api::Rpc, "rpc".parse().unwrap());
		assert_eq!(Api::SecretStore, "secretstore".parse().unwrap());
		assert_eq!(Api::Private, "private".parse().unwrap());
		assert_eq!(Api::TetsyTransactionsPool, "tetsy_transactions_pool".parse().unwrap());
	...
	fn test_api_set_parsing() {
		assert_eq!(
			ApiSet::List(vec![Api::Web3, Api::Vap].into_iter().collect()),
			"web3,vap".parse().unwrap()
		);
	}
