...
usage! {
	...
	}
	{
		...
		["Operating Options"]
			FLAG flag_no_download: (bool) = false, or |c: &Config| c.tetsy.as_ref()?.no_download.clone(),
			"--no-download",
			"Normally new releases will be downloaded ready for updating. This disables it. Not recommended.",

			FLAG flag_no_consensus: (bool) = false, or |c: &Config| c.tetsy.as_ref()?.no_consensus.clone(),
			"--no-consensus",
			"Force the binary to run even if there are known issues regarding consensus. Not recommended.",

			FLAG flag_light: (bool) = false, or |c: &Config| c.tetsy.as_ref()?.light,
			"--light",
			...
			FLAG flag_no_hardcoded_sync: (bool) = false, or |c: &Config| c.tetsy.as_ref()?.no_hardcoded_sync,
			"--no-hardcoded-sync",
			...
			"--force-direct",
			"Run the originally installed version of Tetsy, ignoring any updates that have since been installed.",

			ARG arg_mode: (String) = "last", or |c: &Config| c.tetsy.as_ref()?.mode.clone(),
			"--mode=[MODE]",
			...
			ARG arg_mode_timeout: (u64) = 300u64, or |c: &Config| c.tetsy.as_ref()?.mode_timeout.clone(),
			"--mode-timeout=[SECS]",
			"Specify the number of seconds before inactivity timeout occurs when mode is dark or passive",

			ARG arg_mode_alarm: (u64) = 3600u64, or |c: &Config| c.tetsy.as_ref()?.mode_alarm.clone(),
			"--mode-alarm=[SECS]",
			"Specify the number of seconds before auto sleep reawake timeout occurs when mode is passive",

			ARG arg_auto_update: (String) = "critical", or |c: &Config| c.tetsy.as_ref()?.auto_update.clone(),
			"--auto-update=[SET]",
			...
			ARG arg_auto_update_delay: (u16) = 100u16, or |c: &Config| c.tetsy.as_ref()?.auto_update_delay.clone(),
			"--auto-update-delay=[NUM]",
			"Specify the maximum number of blocks used for randomly delaying updates.",

			ARG arg_auto_update_check_frequency: (u16) = 20u16, or |c: &Config| c.tetsy.as_ref()?.auto_update_check_frequency.clone(),
			"--auto-update-check-frequency=[NUM]",
			"Specify the number of blocks between each auto-update check.",

			ARG arg_release_track: (String) = "current", or |c: &Config| c.tetsy.as_ref()?.release_track.clone(),
			"--release-track=[TRACK]",
			...
			ARG arg_chain: (String) = "foundation", or |c: &Config| c.tetsy.as_ref()?.chain.clone(),
			"--chain=[CHAIN]",
			"Specify the blockchain type. CHAIN may be either a JSON chain specification file or vapory, classic, poacore, xdai, volta, ewc, musicoin, ellaism, mix, callisto, vaporcore, morden, mordor, ropsten, kovan, rinkeby, goerli, kotti, poasokol, testnet, evantestcore, evancore or dev.",

			ARG arg_keys_path: (String) = "$BASE/keys", or |c: &Config| c.tetsy.as_ref()?.keys_path.clone(),
			"--keys-path=[PATH]",
			"Specify the path for JSON key files to be found",

			ARG arg_identity: (String) = "", or |c: &Config| c.tetsy.as_ref()?.identity.clone(),
			"--identity=[NAME]",
			"Specify your node's name.",

			ARG arg_base_path: (Option<String>) = None, or |c: &Config| c.tetsy.as_ref()?.base_path.clone(),
			"-d, --base-path=[PATH]",
			"Specify the base data storage path.",

			ARG arg_db_path: (Option<String>) = None, or |c: &Config| c.tetsy.as_ref()?.db_path.clone(),
			"--db-path=[PATH]",
		...
		["API and Console Options – HTTP JSON-RPC"]
			...
			ARG arg_jsonrpc_apis: (String) = "web3,vap,pubsub,net,tetsy,private,tetsy_pubsub,traces,rpc,tetsy_transactions_pool", or |c: &Config| c.rpc.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--jsonrpc-apis=[APIS]",
			"Specify the APIs available through the HTTP JSON-RPC interface using a comma-delimited list of API names. Possible names are: all, safe, debug, web3, net, vap, pubsub, personal, signer, tetsy, tetsy_pubsub, tetsy_accounts, tetsy_set, traces, rpc, secretstore. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: web3, net, vap, pubsub, tetsy, tetsy_pubsub, traces, rpc",
		...
		["API and Console Options – WebSockets"]
			...
			ARG arg_ws_apis: (String) = "web3,vap,pubsub,net,tetsy,tetsy_pubsub,private,traces,rpc,tetsy_transactions_pool", or |c: &Config| c.websockets.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--ws-apis=[APIS]",
			"Specify the JSON-RPC APIs available through the WebSockets interface using a comma-delimited list of API names. Possible names are: all, safe, web3, net, vap, pubsub, personal, signer, tetsy, tetsy_pubsub, tetsy_accounts, tetsy_set, traces, rpc, secretstore. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: web3, net, vap, pubsub, tetsy, tetsy_pubsub, traces, rpc",

			ARG arg_ws_origins: (String) = "tetsy://*,chrome-extension://*,moz-extension://*", or |c: &Config| c.websockets.as_ref()?.origins.as_ref().map(|vec| vec.join(",")),
			"--ws-origins=[URL]",
		...
		["API and Console Options – IPC"]
			...
			ARG arg_ipc_apis: (String) = "web3,vap,pubsub,net,tetsy,tetsy_pubsub,tetsy_accounts,private,traces,rpc,tetsy_transactions_pool", or |c: &Config| c.ipc.as_ref()?.apis.as_ref().map(|vec| vec.join(",")),
			"--ipc-apis=[APIS]",
			"Specify custom API set available via JSON-RPC over IPC using a comma-delimited list of API names. Possible names are: all, safe, web3, net, vap, pubsub, personal, signer, tetsy, tetsy_pubsub, tetsy_accounts, tetsy_set, traces, rpc, secretstore. You can also disable a specific API by putting '-' in the front, example: all,-personal. 'safe' enables the following APIs: web3, net, vap, pubsub, tetsy, tetsy_pubsub, traces, rpc",
		...
		["Sealing/Mining Options"]
			...
			FLAG flag_no_persistent_txqueue: (bool) = false, or |c: &Config| c.tetsy.as_ref()?.no_persistent_txqueue,
			...
			ARG arg_usd_per_vap: (String) = "auto", or |c: &Config| c.mining.as_ref()?.usd_per_vap.clone(),
			"--usd-per-vap=[SOURCE]",
			"USD value of a single VAP. SOURCE may be either an amount in USD, a web service or 'auto' to use each web service in turn and fallback on the last known good value.",
		...
		["Whisper Options"]
			...
			"--whisper",
			"Does nothing. Whisper has been moved to https://github.com/tetcoin/whisper",
		...
		["Legacy Options"]
			...
			FLAG flag_gvap: (bool) = false, or |_| None,
			"--gvap",
			"Run in Gvap-compatibility mode. Sets the IPC path to be the same as Gvap's. Overrides the --ipc-path and --ipcpath options. Alters RPCs to reflect Gvap bugs. Includes the personal_ RPC by default.",

			FLAG flag_import_gvap_keys: (bool) = false, or |_| None,
			"--import-gvap-keys",
			"Attempt to import keys from Gvap client.",
			...
			"--public-node",
			"Does nothing; Public node is removed from Tetsy.",
			...
			ARG arg_vaporbase: (Option<String>) = None, or |_| None,
			"--vaporbase=[ADDRESS]",
...
mod tests {
	...
	fn should_accept_any_argument_order() {
		let args = Args::parse(&["tetsy", "--no-warp", "account", "list"]).unwrap();
		assert_eq!(args.flag_no_warp, true);

		let args = Args::parse(&["tetsy", "account", "list", "--no-warp"]).unwrap();
		assert_eq!(args.flag_no_warp, true);

		let args = Args::parse(&["tetsy", "--chain=dev", "account", "list"]).unwrap();
		assert_eq!(args.arg_chain, "dev");

		let args = Args::parse(&["tetsy", "account", "list", "--chain=dev"]).unwrap();
		assert_eq!(args.arg_chain, "dev");
	}

	#[test]
	fn should_not_crash_on_warp() {
		let args = Args::parse(&["tetsy", "--warp"]);
		assert!(args.is_ok());

		let args = Args::parse(&["tetsy", "account", "list", "--warp"]);
		assert!(args.is_ok());
	}

	#[test]
	fn should_reject_invalid_values() {
		let args = Args::parse(&["tetsy", "--cache=20"]);
		assert!(args.is_ok());

		let args = Args::parse(&["tetsy", "--cache=asd"]);
		assert!(args.is_err());
	}

	#[test]
	fn should_parse_args_and_flags() {
		let args = Args::parse(&["tetsy", "--no-warp"]).unwrap();
		assert_eq!(args.flag_no_warp, true);

		let args = Args::parse(&["tetsy", "--pruning", "archive"]).unwrap();
		assert_eq!(args.arg_pruning, "archive");

		let args = Args::parse(&["tetsy", "export", "state", "--no-storage"]).unwrap();
		assert_eq!(args.flag_export_state_no_storage, true);

		let args = Args::parse(&["tetsy", "export", "state", "--min-balance","123"]).unwrap();
		assert_eq!(args.arg_export_state_min_balance, Some("123".to_string()));
	}

	#[test]
	fn should_exit_gracefully_on_unknown_argument() {
		let result = Args::parse(&["tetsy", "--please-exit-gracefully"]);
		assert!(
			match result {
	...
	fn should_use_subcommand_arg_default() {
		let args = Args::parse(&["tetsy", "export", "state", "--at", "123"]).unwrap();
		assert_eq!(args.arg_export_state_at, "123");
		assert_eq!(args.arg_snapshot_at, "latest");

		let args = Args::parse(&["tetsy", "snapshot", "--at", "123", "file.dump"]).unwrap();
		assert_eq!(args.arg_snapshot_at, "123");
		assert_eq!(args.arg_export_state_at, "latest");

		let args = Args::parse(&["tetsy", "export", "state"]).unwrap();
		assert_eq!(args.arg_snapshot_at, "latest");
		assert_eq!(args.arg_export_state_at, "latest");

		let args = Args::parse(&["tetsy", "snapshot", "file.dump"]).unwrap();
		assert_eq!(args.arg_snapshot_at, "latest");
		assert_eq!(args.arg_export_state_at, "latest");
	}
	...
	fn should_parse_multiple_values() {
		let args = Args::parse(&["tetsy", "account", "import", "~/1", "~/2"]).unwrap();
		assert_eq!(args.arg_account_import_path, Some(vec!["~/1".to_owned(), "~/2".to_owned()]));

		let args = Args::parse(&["tetsy", "account", "import", "~/1,ext"]).unwrap();
		assert_eq!(args.arg_account_import_path, Some(vec!["~/1,ext".to_owned()]));

		let args = Args::parse(&["tetsy", "--secretstore-nodes", "abc@127.0.0.1:3333,cde@10.10.10.10:4444"]).unwrap();
		assert_eq!(args.arg_secretstore_nodes, "abc@127.0.0.1:3333,cde@10.10.10.10:4444");

		let args = Args::parse(&["tetsy", "--password", "~/.safe/1", "--password", "~/.safe/2", "--ui-port", "8123"]).unwrap();
		assert_eq!(args.arg_password, vec!["~/.safe/1".to_owned(), "~/.safe/2".to_owned()]);
		assert_eq!(args.arg_ui_port, Some(8123));

		let args = Args::parse(&["tetsy", "--password", "~/.safe/1,~/.safe/2", "--ui-port", "8123"]).unwrap();
		assert_eq!(args.arg_password, vec!["~/.safe/1".to_owned(), "~/.safe/2".to_owned()]);
		assert_eq!(args.arg_ui_port, Some(8123));
	}
	...
	fn should_enable_stratum_by_param() {
		...
		// when
		let args = Args::parse_with_config(&["tetsy", "--stratum"], config).unwrap();
	...
	fn should_parse_full_config() {
		...
		assert_eq!(args, Args {
			...
			arg_jsonrpc_apis: "web3,vap,net,tetsy,traces,rpc,secretstore".into(),
			...
			arg_ws_apis: "web3,vap,net,tetsy,traces,rpc,secretstore".into(),
			...
			arg_log_file: Some("/var/log/tetsy.log".into()),
			flag_no_color: false,
	...
	fn should_deserialize_toml_file() {
		...
		assert_eq!(config, Config {
			...
			ipc: Some(Ipc {
				...
				path: None,
				chmod: None,
				apis: Some(vec!["rpc".into(), "vap".into()]),
			}),
			dapps: Some(Dapps {
			...
			misc: Some(Misc {
				...
				log_file: Some("/var/log/tetsy.log".into()),
				color: Some(true),
				ports_shift: Some(0),
	...
	fn should_not_accept_min_peers_bigger_than_max_peers() {
		match Args::parse(&["tetsy", "--max-peers=39", "--min-peers=40"]) {
	...
	fn should_accept_max_peers_equal_or_bigger_than_min_peers() {
		Args::parse(&["tetsy", "--max-peers=40", "--min-peers=40"]).unwrap();
		Args::parse(&["tetsy", "--max-peers=100", "--min-peers=40"]).unwrap();
	}
}
