...
pub enum AccountCmd {
	...
	ImportFromGvap(ImportFromGvapAccounts)
}
...
/// Parameters for gvap accounts' import
#[derive(Debug, PartialEq)]
pub struct ImportFromGvapAccounts {
...
mod command {
	...
	use vapstore::{VapStore, SecretStore, SecretVaultRef, import_account, import_accounts, read_gvap_accounts};
	...
	pub fn execute(cmd: AccountCmd) -> Result<String, String> {
		match cmd {
			...
			AccountCmd::ImportFromGvap(import_gvap_cmd) => import_gvap(import_gvap_cmd)
		}
	}
	...
	fn secret_store(dir: Box<RootDiskDirectory>, iterations: Option<u32>) -> Result<VapStore, String> {
		match iterations {
			Some(i) => VapStore::open_with_iterations(dir, i),
			_ => VapStore::open(dir)
	...
	fn import_gvap(i: ImportFromGvapAccounts) -> Result<String, String> {
		use std::io::ErrorKind;
		use vapstore::Error;

		let dir = Box::new(keys_dir(i.to, i.spec)?);
		let secret_store = Box::new(secret_store(dir, None)?);
		let gvap_accounts = read_gvap_accounts(i.testnet);
		match secret_store.import_gvap_accounts(SecretVaultRef::Root, gvap_accounts, i.testnet) {
			Ok(v) => Ok(format!("Successfully imported {} account(s) from gvap.", v.len())),
			Err(Error::Io(ref io_err)) if io_err.kind() == ErrorKind::NotFound => Err("Failed to find gvap keys folder.".into()),
			Err(err) => Err(format!("Import gvap accounts failed. {}", err))
		}
	}
}
