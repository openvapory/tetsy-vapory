...
use vapcore::miner::{Miner, MinerService};
use tetsy_crypto::publickey::Error as VapKeyError;
use sync::SyncProvider;
use tetsy_registrar::RegistrarClient;
use vapcore_secretstore::{BlockId, BlockNumber, SecretStoreChain, NewBlocksNotify, SigningKeyPair, ContractAddress, Filter};
...
fn into_vapcore_block_id(id: BlockId) -> VapcoreBlockId {
	match id {
		BlockId::Hash(hash) => VapcoreBlockId::Hash(hash),
		BlockId::Number(number) => VapcoreBlockId::Number(number),
		BlockId::Earliest => VapcoreBlockId::Earliest,
		BlockId::Latest => VapcoreBlockId::Latest,
	}
}
...
impl SecretStoreChain for TrustedClient {
	...
	fn transact_contract(&self, contract: Address, tx_data: Bytes) -> Result<(), VapKeyError> {
		let client = self.client.upgrade().ok_or_else(|| VapKeyError::Custom("cannot submit tx when client is offline".into()))?;
		let miner = self.miner.upgrade().ok_or_else(|| VapKeyError::Custom("cannot submit tx when miner is offline".into()))?;
		...
		miner.import_own_transaction(&*client, signed.into())
			.map_err(|e| VapKeyError::Custom(format!("failed to import tx: {}", e)))
	}
		...
		match *address {
			...
			ContractAddress::Registry => self.get_trusted().and_then(|client|
				self.get_confirmed_block_hash()
					.and_then(|block| {
						client.get_address(registry_name, VapcoreBlockId::Hash(block))
							.unwrap_or(None)
					})
			),
		}
	...
	fn call_contract(&self, block_id: BlockId, contract_address: Address, data: Bytes) -> Result<Bytes, String> {
		if let Some(client) = self.get_trusted() {
			client.call_contract(into_vapcore_block_id(block_id), contract_address, data)
		} else {
	...
	fn block_hash(&self, id: BlockId) -> Option<H256> {
		if let Some(client) = self.get_trusted() {
			client.block_hash(into_vapcore_block_id(id))
		} else {
			None
		}
	}
	...
	fn block_number(&self, id: BlockId) -> Option<BlockNumber> {
		if let Some(client) = self.get_trusted() {
			client.block_number(into_vapcore_block_id(id))
		} else {
			None
		}
	}
	...
	fn retrieve_last_logs(&self, filter: Filter) -> Option<Vec<RawLog>> {
		...
		self.logs(BlockchainFilter {
			from_block: VapcoreBlockId::Hash(first_block),
			to_block: VapcoreBlockId::Hash(confirmed_block),
