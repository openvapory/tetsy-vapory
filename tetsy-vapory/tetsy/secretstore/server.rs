...
mod server {
	...
	fn into_service_contract_address(address: ContractAddress) -> vapcore_secretstore::ContractAddress {
		match address {
			ContractAddress::Registry => vapcore_secretstore::ContractAddress::Registry,
			ContractAddress::Address(address) => vapcore_secretstore::ContractAddress::Address(address),
		}
	}

	/// Key server
	pub struct KeyServer {
		_key_server: Box<dyn vapcore_secretstore::KeyServer>,
	}

	impl KeyServer {
		/// Create new key server
		pub fn new(mut conf: Configuration, deps: Dependencies, executor: Executor) -> Result<Self, String> {
			let self_secret: Arc<dyn vapcore_secretstore::SigningKeyPair> = match conf.self_secret.take() {
				Some(NodeSecretKey::Plain(secret)) => Arc::new(vapcore_secretstore::PlainNodeKeyPair::new(
			...
			let mut cconf = vapcore_secretstore::ServiceConfiguration {
				listener_address: if conf.http_enabled { Some(vapcore_secretstore::NodeAddress {
				...
				cluster_config: vapcore_secretstore::ClusterConfiguration {
					listener_address: vapcore_secretstore::NodeAddress {
						address: conf.interface.clone(),
						port: conf.port,
					},
					nodes: conf.nodes.into_iter().map(|(p, (ip, port))| (p, vapcore_secretstore::NodeAddress {
						address: ip,
						port: port,
					})).collect(),
			...
			let db = vapcore_secretstore::open_secretstore_db(&conf.data_path)?;
			let trusted_client = TrustedClient::new(self_secret.clone(), deps.client, deps.sync, deps.miner);
			let key_server = vapcore_secretstore::start(trusted_client, self_secret, cconf, db, executor)
