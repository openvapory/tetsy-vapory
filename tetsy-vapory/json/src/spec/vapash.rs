...
/// Deserializable doppelganger of block rewards for VapashParams
...
/// Vapash engine deserialization.
#[derive(Debug, PartialEq, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Vapash {
	/// Vapash params.
	pub params: VapashParams,
}

#[cfg(test)]
mod tests {
	use std::str::FromStr;
	use super::{Address, BlockReward, Vapash, VapashParams, Uint};
	use vapory_types::{H160, U256};

	#[test]
	fn vapash_deserialization() {
		let s = r#"{
			"params": {
		...
		let deserialized: Vapash = serde_json::from_str(s).unwrap();

		assert_eq!(deserialized, Vapash {
			params: VapashParams {
	...
	#[test]
	fn vapash_deserialization_missing_optionals() {
		let s = r#"{
			"params": {
				"difficultyBoundDivisor": "0x0800",
				"minimumDifficulty": "0x020000"
	...
	fn test_zero_value_divisor() {
		...
		let _deserialized: Vapash = serde_json::from_str(s).unwrap();
	}
}
