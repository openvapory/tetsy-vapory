...
if [ -x "$(command -v vapvm)" ]; then
  vapvm --code $CODE1
  echo "^^^^ vapvm"
fi
./target/release/tetsy-vvm stats --code $CODE1 --gas 4402000
echo "^^^^ usize"
./target/release/tetsy-vvm stats --code $CODE1
echo "^^^^ U256"
...
./target/release/tetsy-vvm stats --code $CODE2 --gas 143020115
echo "^^^^ usize"
./target/release/tetsy-vvm stats --code $CODE2
echo "^^^^ U256"
