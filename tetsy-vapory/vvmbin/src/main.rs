...
//! Tetsy VVM Interpreter Binary.
//!
//! ## Overview
//!
//! The Tetsy VVM interpreter binary is a tool in the Tetsy
//! Vapory toolchain. It is an VVM implementation for Tetsy Vapory that
//! is used to run a standalone version of the VVM interpreter.
//!
//! ## Usage
//!
//! The vvmbin tool is not distributed with regular Tetsy Vapory releases
//! so you need to build it from source and run it like so:
//!
//! ```bash
//! cargo build -p vvmbin --release
//! ./target/release/tetsy-vvm --help
//! ```
...
Usage:
    tetsy-vvm state-test <file> [--chain CHAIN --only NAME --json --std-json --std-dump-json --std-out-only --std-err-only]
...
fn run_state_test(args: Args) {
	use vapjson::test_helpers::state::Test;
...
mod tests {
	...
	use docopt::Docopt;
	use vapcore::test_helpers::TrieSpec;
	use vapjson::test_helpers::state::State;
	use serde::Deserialize;
	...
	fn should_error_out_of_gas() {
		...
		assert!(
			&String::from_utf8_lossy(&**res.0.lock().unwrap())
				.starts_with(r#"{"error":"VVM: Out of gas","gasUsed":"0x1","#),
		);
	}
