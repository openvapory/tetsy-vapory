...
impl PodAccount {
	...
	pub fn rlp(&self) -> Bytes {
		...
		stream.append(&sec_trie_root(self.storage.iter().map(|(k, v)| (k, tetsy_rlp::encode(&v.into_uint())))));
	...
	pub fn insert_additional(&self, db: &mut dyn HashDB<KeccakHasher, DBValue>, factory: &TrieFactory<Layout>) {
		match self.code {
			Some(ref c) if !c.is_empty() => { db.insert(tetsy_hash_db::EMPTY_PREFIX, c); }
			_ => {}
		}
		let mut r = H256::zero();
		let mut t = factory.create(db, &mut r);
		for (k, v) in &self.storage {
			if let Err(e) = t.insert(k.as_bytes(), &tetsy_rlp::encode(&v.into_uint())) {
...
impl From<vapjson::spec::Account> for PodAccount {
	fn from(a: vapjson::spec::Account) -> Self {
		PodAccount {
