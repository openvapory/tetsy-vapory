...
fn snap_and_restore() {
	...
	let mut old_db = journaldb::new_tetsy_memory_db();
	let db_cfg = DatabaseConfig::with_columns(vapcore_db::NUM_COLUMNS);
	...
	let new_db = journaldb::new(db, Algorithm::OverlayRecent,  vapcore_db::COL_STATE);
...
fn get_code_from_prev_chunk() {
	...
	let acc: BasicAccount = ::tetsy_rlp::decode(&thin_rlp).expect("error decoding basic account");
	...
	let state_db = journaldb::new(new_db, Algorithm::OverlayRecent, vapcore_db::COL_STATE);
