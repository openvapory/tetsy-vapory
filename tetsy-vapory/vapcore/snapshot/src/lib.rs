...
//! https://wiki.tetcoin.org/Warp-Sync-Snapshot-Format
...
use tetsy_keccak_hash::{keccak, KECCAK_NULL_RLP, KECCAK_EMPTY};
...
use common_types::{
	ids::BlockId,
	header::Header,
	errors::{SnapshotError as Error, VapcoreError},
...
use tetsy_kvdb::{KeyValueDB, DBValue};
...
pub fn chunk_state<'a>(
	...
	for item in account_iter {
		...
		let account = ::tetsy_rlp::decode(&*account_data)?;
...
impl StateRebuilder {
	...
	pub fn new(db: Arc<dyn KeyValueDB>, pruning: Algorithm) -> Self {
		StateRebuilder {
			db: journaldb::new(db.clone(), pruning, vapcore_db::COL_STATE),
	...
	pub fn feed(&mut self, chunk: &[u8], flag: &AtomicBool) -> Result<(), VapcoreError> {
		...
		for (code_hash, code, first_with) in status.new_code {
			for addr_hash in self.missing_code.remove(&code_hash).unwrap_or_else(Vec::new) {
				...
				db.emplace(code_hash, tetsy_hash_db::EMPTY_PREFIX, code.to_vec());
			}
	...
	pub fn finalize(mut self, era: u64, id: H256) -> Result<Box<dyn JournalDB>, VapcoreError> {
...
fn rebuild_accounts(
...
) -> Result<RebuiltStatus, VapcoreError> {
	...
	for (account_rlp, out) in account_fat_rlps.into_iter().zip(out_chunk.iter_mut()) {
		...
		let thin_rlp = {
						...
						match known_code.get(&code_hash) {
							Some(&first_with) => {
								...
								let code = AccountDB::from_hash(db, first_with)
									.get(&code_hash, tetsy_hash_db::EMPTY_PREFIX)
									.ok_or_else(|| Error::MissingCode(vec![first_with]))?;

								// and write it again under a different mangled key
								AccountDBMut::from_hash(db, hash).emplace(code_hash, tetsy_hash_db::EMPTY_PREFIX, code);
							}
			...
			::tetsy_rlp::encode(&acc)
		};

		*out = (hash, thin_rlp);
	}
	if let Some(&(ref hash, ref rlp)) = out_chunk.iter().last() {
		known_storage_roots.insert(*hash, ::tetsy_rlp::decode::<BasicAccount>(rlp)?.storage_root);
	}
...
pub fn verify_old_block(rng: &mut OsRng, header: &Header, engine: &dyn Engine, chain: &BlockChain, always: bool) -> Result<(), VapcoreError> {
