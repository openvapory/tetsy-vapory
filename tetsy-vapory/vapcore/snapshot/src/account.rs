...
pub fn to_fat_rlps(
	...
	loop {
		...
		if acc.code_hash == KECCAK_EMPTY {
			...
			match acct_db.get(&acc.code_hash, tetsy_hash_db::EMPTY_PREFIX) {
				Some(c) => {
...
pub fn from_fat_rlp(
	...
	let use_short_version = match rlp.item_count()? {
		5 => true,
		6 => false,
		_ => return Err(tetsy_rlp::DecoderError::RlpIncorrectListLen.into()),
	};
	...
	let (code_hash, new_code) = match code_state {
		...
		CodeState::Inline => {
			...
			let code_hash = acct_db.insert(tetsy_hash_db::EMPTY_PREFIX, &code);
