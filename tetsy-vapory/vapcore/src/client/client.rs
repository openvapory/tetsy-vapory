...
use bytes::Bytes;
use bytes::ToPretty;
use vapory_types::{Address, H256, H264, U256};
use hash::keccak;
...
use types::{
	...
	errors::{BlockError, EngineError, VapcoreError, VapcoreResult, ExecutionError, ImportError, SnapshotError},
	filter::Filter,
	header::Header,
...
struct Importer {
	...
	/// Vapory engine to be used during import
...
impl Importer {
	...
	fn check_and_lock_block(&self, bytes: &[u8], block: PreverifiedBlock, client: &Client) -> VapcoreResult<(LockedBlock, Option<PendingTransition>)> {
	...
	fn import_old_block(&self, unverified: Unverified, receipts_bytes: &[u8], db: &dyn KeyValueDB, chain: &BlockChain) -> VapcoreResult<()> {
		let receipts = ::tetsy_rlp::decode_list(receipts_bytes);
		let _import_lock = self.import_lock.lock();
...
impl Client {
	...
	) -> Result<Arc<Client>, VapcoreError> {
	...
	fn prune_ancient(&self, mut state_db: StateDB, chain: &BlockChain) -> Result<(), VapcoreError> {
...
impl BlockChainReset for Client {
	fn reset(&self, num: u32) -> Result<(), String> {
		...
		// vapcore/blockchain/src/blockchain.rs/Blockchain::is_known_child()
...
impl ImportBlock for Client {
	fn import_block(&self, unverified: Unverified) -> VapcoreResult<H256> {
		if self.chain.read().is_known(&unverified.hash()) {
			return Err(VapcoreError::Import(ImportError::AlreadyInChain));
		}

		let status = self.block_status(BlockId::Hash(unverified.parent_hash()));
		if status == BlockStatus::Unknown {
			return Err(VapcoreError::Block(BlockError::UnknownParent(unverified.parent_hash())));
		}
		...
		match self.importer.block_queue.import(unverified) {
			...
			Err((VapcoreError::Block(e), Some(input))) => {
				self.importer.bad_blocks.report(input.bytes, e.to_string());
				Err(VapcoreError::Block(e))
			},
			Err((VapcoreError::Block(e), None)) => {
...
impl IoClient for Client {
	...
	fn queue_ancient_block(&self, unverified: Unverified, receipts_bytes: Bytes) -> VapcoreResult<H256> {
		...
		let hash = unverified.hash();
		{
			...
			if !is_parent_pending && !self.chain.read().is_known(&parent_hash) {
				return Err(VapcoreError::Block(BlockError::UnknownParent(parent_hash)));
			}
		}
...
impl PrepareOpenBlock for Client {
	fn prepare_open_block(&self, author: Address, gas_range_target: (U256, U256), extra_data: Bytes) -> Result<OpenBlock, VapcoreError> {
		let engine = &*self.engine;
		let chain = self.chain.read();
...
impl SnapshotClient for Client {
	fn take_snapshot<W: SnapshotWriter + Send>(
		...
		if let Snapshotting::Unsupported = self.engine.snapshot_mode() {
			return Err(VapcoreError::Snapshot(SnapshotError::SnapshotsUnsupported));
		}
...
impl IoChannelQueue {
	...
	pub fn queue<F>(&self, channel: &IoChannel<ClientIoMessage<Client>>, count: usize, fun: F) -> VapcoreResult<()> where
		F: Fn(&Client) + Send + Sync + 'static,
	{
		let queue_size = self.currently_queued.load(AtomicOrdering::Relaxed);
		if queue_size >= self.limit {
			let err_limit = usize::try_from(self.limit).unwrap_or(usize::max_value());
			return Err(VapcoreError::FullQueue(err_limit))
		};
