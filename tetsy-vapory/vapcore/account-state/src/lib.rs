...
//! Note: the code that needs access to `vapcore` types such as `Machine` and `Executive` is found in
//! the `executive_state` module in `vapcore`. Most tests for the `State` module in this crate are
