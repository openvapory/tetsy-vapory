...
use vapcore_db::cache_manager::CacheManager;
use vapcore_db::keys::{BlockReceipts, BlockDetails, TransactionAddress, EPOCH_KEY_PREFIX, EpochTransitions};
use vapcore_db::{self as db, Writable, Readable, CacheUpdatePolicy};
use vapory_types::{H256, Bloom, BloomRef, U256};
...
impl<'a> Iterator for EpochTransitionIter<'a> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		loop {
			...
			let transitions: EpochTransitions = ::tetsy_rlp::decode(&val[..]).expect("decode error: the db is corrupted or the data structure has changed");
...
mod tests {
	...
	pub fn new_db() -> Arc<dyn BlockChainDB> {
		...
		let db = TestBlockChainDB {
			...
			key_value: Arc::new(tetsy_kvdb_memorydb::create(vapcore_db::NUM_COLUMNS))
		};
