...
fn private_contract() {
	...
	let signatures: Vec<_> = [&key3, &key4].iter().map(|k|
		Signature::from(tetsy_crypto::publickey::sign(&k.secret(), &nonced_state_hash).unwrap().into_electrum())).collect();
	...
	let signatures: Vec<_> = [&key4].iter().map(|k|
		Signature::from(tetsy_crypto::publickey::sign(&k.secret(), &private_state_hash).unwrap().into_electrum())).collect();
