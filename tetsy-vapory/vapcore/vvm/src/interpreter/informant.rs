...
#[macro_use]
#[cfg(not(feature = "vvm-debug"))]
mod inner {
	macro_rules! vvm_debug {
		($x: expr) => {}
	}

	pub struct VvmInformant;
	impl VvmInformant {
...
mod inner {
	...
	fn print(data: String) {
		if cfg!(feature = "vvm-debug-tests") {
			println!("{}", data);
		} else {
			debug!(target: "vvm", "{}", data);
		}
	}
	...
	impl Stats {
		...
		fn avg(&self) -> u64 {
			VvmInformant::as_micro(&self.total_duration) / self.count
		}
	}
}
