...
use crate::{
	...
	chain::{
		...
		ChainSync, SyncSupplier, VAP_PROTOCOL_VERSION_63, PAR_PROTOCOL_VERSION_4
	},
...
use vapcore::{
	client::{Client as VapcoreClient, ClientConfig},
...
impl FlushingBlockChainClient for VapcoreClient {
	fn flush(&self) {
		self.flush_queue();
	}
}
...
impl<'p, C> SyncIo for TestIo<'p, C> where C: FlushingBlockChainClient, C: 'p {
	...
	fn vap_protocol_version(&self, _peer: PeerId) -> u8 {
		VAP_PROTOCOL_VERSION_63.0
	}

	fn protocol_version(&self, protocol: &ProtocolId, peer_id: PeerId) -> u8 {
		if protocol == &WARP_SYNC_PROTOCOL_ID { PAR_PROTOCOL_VERSION_4.0 } else { self.vap_protocol_version(peer_id) }
	}
...
pub struct VapPeer<C> where C: FlushingBlockChainClient {
	pub chain: Arc<C>,
	pub miner: Arc<Miner>,
...
impl<C> VapPeer<C> where C: FlushingBlockChainClient {
...
impl<C: FlushingBlockChainClient> Peer for VapPeer<C> {
...
impl TestNet<VapPeer<TestBlockChainClient>> {
	...
	pub fn new_with_config(n: usize, config: SyncConfig) -> Self {
		...
		for _ in 0..n {
			...
			net.peers.push(Arc::new(VapPeer {
	...
	pub fn peer_mut(&mut self, i: usize) -> &mut VapPeer<TestBlockChainClient> {
		Arc::get_mut(&mut self.peers[i]).expect("Arc never exposed externally")
	}
}

impl TestNet<VapPeer<VapcoreClient>> {
	pub fn with_spec<F>(
		n: usize,
		config: SyncConfig,
		spec_factory: F,
...
impl<C: FlushingBlockChainClient> TestNet<VapPeer<C>> {
...
pub struct TestIoHandler {
	pub client: Arc<VapcoreClient>,
	pub private_tx_queued: Mutex<usize>,
}

impl TestIoHandler {
	pub fn new(client: Arc<VapcoreClient>) -> Self {
		TestIoHandler {
			client,
...
impl IoHandler<ClientIoMessage<VapcoreClient>> for TestIoHandler {
	fn message(&self, _io: &IoContext<ClientIoMessage<VapcoreClient>>, net_message: &ClientIoMessage<VapcoreClient>) {
		match *net_message {
...
impl ChainNotify for VapPeer<VapcoreClient> {
