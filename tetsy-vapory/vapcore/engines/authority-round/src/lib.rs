...
use rand::rngs::OsRng;
use tetsy_rlp::{encode, Decodable, DecoderError, Encodable, RlpStream, Rlp};
use vapory_types::{H256, H520, Address, U128, U256};
...
impl From<vapjson::spec::AuthorityRoundParams> for AuthorityRoundParams {
	fn from(p: vapjson::spec::AuthorityRoundParams) -> Self {
		let map_step_duration = |u: vapjson::uint::Uint| {
		...
		let step_durations: BTreeMap<_, _> = match p.step_duration {
			vapjson::spec::StepDuration::Single(u) =>
				iter::once((0,  map_step_duration(u))).collect(),
			vapjson::spec::StepDuration::Transitions(tr) => {
				if tr.is_empty() {
		...
		let randomness_contract_address = p.randomness_contract_address.map_or_else(BTreeMap::new, |transitions| {
			transitions.into_iter().map(|(vapjson::uint::Uint(block), addr)| {
...
impl EmptyStep {
	...
	fn verify(&self, validators: &dyn ValidatorSet) -> Result<bool, Error> {
		...
		tetsy_crypto::publickey::verify_address(&correct_proposer, &self.signature.into(), &message)
			.map_err(|e| e.into())
	}

	fn author(&self) -> Result<Address, Error> {
		let message = keccak(empty_step_rlp(self.step, &self.parent_hash));
		let public = tetsy_crypto::publickey::recover(&self.signature.into(), &message)?;
		Ok(tetsy_crypto::publickey::public_to_address(&public))
	}
...
fn header_step(header: &Header, empty_steps_transition: u64) -> Result<u64, ::tetsy_rlp::DecoderError> {
...
fn header_signature(header: &Header, empty_steps_transition: u64) -> Result<Signature, ::tetsy_rlp::DecoderError> {
...
// (i.e. header.number() >= self.empty_steps_transition).
fn header_empty_steps(header: &Header) -> Result<Vec<EmptyStep>, ::tetsy_rlp::DecoderError> {
...
fn verify_external(header: &Header, validators: &dyn ValidatorSet, empty_steps_transition: u64) -> Result<(), Error> {
	...
	let is_invalid_proposer = *header.author() != correct_proposer || {
		...
		!tetsy_crypto::publickey::verify_address(&correct_proposer, &proposer_signature, &header_seal_hash)?
	};
...
impl Engine for AuthorityRound {
	...
	fn generate_seal(&self, block: &ExecutedBlock, parent: &Header) -> Seal {
		...
		if is_step_proposer(&*validators, header.parent_hash(), step, header.author()) {
			...
			let empty_steps_rlp = if header.number() >= self.empty_steps_transition {
				...
				Some(::tetsy_rlp::encode_list(&empty_steps))
			} else {
				None
			};
	...
	fn is_epoch_end(
		...
		for finalized_hash in finalized {
			if let Some(pending) = transition_store(*finalized_hash) {
				...
				let finality_proof = ::tetsy_rlp::encode_list(&finality_proof);
	...
	fn sign(&self, hash: H256) -> Result<Signature, Error> {
		Ok(self.signer.read()
			.as_ref()
			.ok_or(tetsy_crypto::publickey::Error::InvalidAddress)?
			.sign(hash)?
		)
	}
...
mod tests {
	...
	use vapory_types::{Address, H520, H256, U256};
	use tetsy_crypto::publickey::Signature;
	use common_types::{
		header::Header,
	...
	fn set_empty_steps_seal(header: &mut Header, step: u64, block_signature: &Signature, empty_steps: &[SealedEmptyStep]) {
		header.set_seal(vec![
			...
			::tetsy_rlp::encode_list(&empty_steps),
		]);
	}
	...
	fn seal_with_empty_steps() {
		...
		if let Seal::Regular(seal) = engine.generate_seal(&b2, &genesis_header) {
			...
			let empty_steps = ::tetsy_rlp::encode_list(&vec![empty_step2]);
	...
	fn seal_empty_block_with_empty_steps() {
		...
		if let Seal::Regular(seal) = engine.generate_seal(&b3, &genesis_header) {
			...
			let empty_steps = ::tetsy_rlp::encode_list(&vec![empty_step2, empty_step3]);
	...
	fn extra_info_from_seal() {
		...
		header.set_seal(vec![
			encode(&2usize),
			encode(&H520::default()),
			::tetsy_rlp::encode_list(&vec![sealed_empty_step]),
		]);
	...
	fn should_collect_block_reward_transitions() {
		...
		let deserialized: vapjson::spec::AuthorityRound = serde_json::from_str(config).unwrap();
