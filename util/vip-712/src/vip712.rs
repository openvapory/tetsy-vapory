...
fn validate_domain(domain: &VIP712Domain) -> Result<(), ValidationError> {
	match (domain.name.as_ref(), domain.version.as_ref(), domain.chain_id, domain.verifying_contract, domain.salt) {
		(None, None, None, None, None) => Err(ValidationError::new("VIP712Domain must include at least one field")),
		_ => Ok(())
	}
}
...
pub struct VIP712 {
	...
	pub(crate) domain: VIP712Domain,
}

impl Validate for VIP712 {
...
mod tests {
	...
	fn test_deserialization() {
		...
		let _ = from_str::<VIP712>(string).unwrap();
	}
	...
	fn test_failing_deserialization() {
		...
		let data = from_str::<VIP712>(string).unwrap();
