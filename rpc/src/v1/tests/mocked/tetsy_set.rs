...
pub type TestTetsySetClient = TetsySetClient<TestBlockChainClient, TestMinerService, TestUpdater, FakeFetch<usize>>;
...
#[test]
fn rpc_tetsy_execute_upgrade() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	let mut io = IoHandler::new();
	io.extend_with(tetsy_set_client(&client, &miner, &updater, &network).to_delegate());

	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_executeUpgrade", "params": [], "id": 1}"#;
...
fn rpc_tetsy_upgrade_ready() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_upgradeReady", "params": [], "id": 1}"#;
...
#[test]
fn rpc_tetsy_set_min_gas_price() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setMinGasPrice", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_tetsy_set_min_gas_price_with_automated_calibration_enabled() {
	let miner = miner_service();
	*miner.min_gas_price.write() = None;
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setMinGasPrice", "params":["0xdeadbeef"], "id": 1}"#;
...
#[test]
fn rpc_tetsy_set_gas_floor_target() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setGasFloorTarget", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_tetsy_set_extra_data() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setExtraData", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_tetsy_set_author() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setAuthor", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_tetsy_set_transactions_limit() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setTransactionsLimit", "params":[10240240], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":false,"id":1}"#;
...
fn rpc_tetsy_set_hash_content() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_hashContent", "params":["https://tetcoin.org/assets/images/vapcore-black-horizontal.png"], "id": 1}"#;
...
#[test]
fn rpc_tetsy_remove_transaction() {
	...
	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_removeTransaction", "params":[""#.to_owned() + &format!("0x{:x}", hash) + r#""], "id": 1}"#;
...
#[test]
fn rpc_tetsy_set_engine_signer() {
	use accounts::AccountProvider;
	use bytes::ToPretty;
	use v1::impls::TetsySetAccountsClient;
	use v1::traits::TetsySetAccounts;
	...
	let mut io = IoHandler::new();
	io.extend_with(
		TetsySetAccountsClient::new(&account_provider, &miner).to_delegate()
	);

	let request = r#"{"jsonrpc": "2.0", "method": "tetsy_setEngineSigner", "params":["0xcd2a3d9f938e13cd947ec05abc7fe734df8dd826", "password"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
