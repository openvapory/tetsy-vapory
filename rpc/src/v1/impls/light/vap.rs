...
//! Vap RPC interface for the light client.
...
use vapory_types::{Address, H64, H160, H256, U64, U256};
use hash::{KECCAK_NULL_RLP, KECCAK_EMPTY_LIST_RLP};
use parking_lot::{RwLock, Mutex};
use tetsy_rlp::Rlp;
use types::transaction::SignedTransaction;
use types::encoded;
use types::filter::Filter as VapcoreFilter;
use types::ids::BlockId;

use v1::impls::vap_filter::Filterable;
...
use v1::types::{
	...
	SyncInfo as RpcSyncInfo, Transaction, CallRequest, Index, Filter, Log, Receipt, Work, VapAccount
};
...
/// Light client `VAP` (and filter) RPC.
pub struct VapClient<C, S: LightSyncProvider + LightNetworkDispatcher + 'static, OD: OnDemandRequester + 'static> {
	sync: Arc<S>,
	client: Arc<C>,
	on_demand: Arc<OD>,
...
impl<C, S, OD> Clone for VapClient<C, S, OD>
where
...
impl<C, S, OD> VapClient<C, S, OD>
where
	C: LightChainClient + 'static,
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static,
	OD: OnDemandRequester + 'static
{
	/// Create a new `VapClient` with a handle to the light sync instance, client,
...
impl<C, S, OD> Vap for VapClient<C, S, OD>
where
	C: LightChainClient + 'static,
	...
	OD: OnDemandRequester + 'static
{
	...
	fn accounts(&self) -> Result<Vec<H160>> {
		self.deprecation_notice.print("vap_accounts", deprecated::msgs::ACCOUNTS);
	...
	fn transaction_by_hash(&self, hash: H256) -> BoxFuture<Option<Transaction>> {
		...
		if in_txqueue {
			...
			// `vap_getTransactionByHash` more than once to ensure the `txqueue` is up to `date` when it is called
	...
	fn proof(&self, _address: H160, _values:Vec<H256>, _num: Option<BlockNumber>) -> BoxFuture<VapAccount> {
		Box::new(future::err(errors::unimplemented(None)))
	}
...
// This trait implementation triggers a blanked impl of `VapFilter`.
impl<C, S, OD> Filterable for VapClient<C, S, OD>
where
	C: LightChainClient + 'static,
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static,
	OD: OnDemandRequester + 'static
{
	...
	fn logs(&self, filter: VapcoreFilter) -> BoxFuture<Vec<Log>> {
		Box::new(self.fetcher().logs(filter)) as BoxFuture<_>
	}

	fn pending_logs(&self, _block_number: u64, _filter: &VapcoreFilter) -> Vec<Log> {
	...
	fn removed_logs(&self, _block_hash: ::vapory_types::H256, _filter: &VapcoreFilter) -> (Vec<Log>, u64) {
		(Default::default(), 0)
	}
}
