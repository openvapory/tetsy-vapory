...
pub mod accounts {
	...
	/// Tetsy-specific account-touching RPC interfaces.
	pub struct TetsySetAccountsClient<M> {
		miner: Arc<M>,
		accounts: Arc<AccountProvider>,
		deprecation_notice: DeprecationNotice,
	}

	impl<M> TetsySetAccountsClient<M> {
		/// Creates new TetsySetAccountsClient
		pub fn new(
			accounts: &Arc<AccountProvider>,
			miner: &Arc<M>,
		) -> Self {
			TetsySetAccountsClient {
	...
	impl<M: MinerService + 'static> TetsySetAccounts for TetsySetAccountsClient<M> {
		fn set_engine_signer(&self, address: H160, password: String) -> Result<bool> {
			self.deprecation_notice.print(
				"tetsy_setEngineSigner",
				"use `tetsy_setEngineSignerSecret` instead. See #9997 for context."
			);
...
impl<C, M, U, F> TetsySetClient<C, M, U, F>
	where C: BlockChainClient + 'static,
{
	/// Creates new `TetsySetClient` with given `Fetch`.
	pub fn new(
		client: &Arc<C>,
		miner: &Arc<M>,
		updater: &Arc<U>,
...
impl<C, M, U, F> TetsySet for TetsySetClient<C, M, U, F> where
	...
	F: Fetch + 'static,
{
	...
	fn set_engine_signer_secret(&self, secret: H256) -> Result<bool> {
		...
		self.miner.set_author(miner::Author::Sealer(enjen::signer::from_keypair(keypair)));
		Ok(true)
	}
