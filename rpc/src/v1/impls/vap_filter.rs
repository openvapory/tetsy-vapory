...
pub trait Filterable {
	...
	fn pending_logs(&self, block_number: u64, filter: &VapcoreFilter) -> Vec<Log>;
	...
	fn removed_logs(&self, block_hash: H256, filter: &VapcoreFilter) -> (Vec<Log>, u64);
}

/// Vap filter rpc implementation for a full node.
pub struct VapFilterClient<C, M> {
	client: Arc<C>,
	miner: Arc<M>,
	polls: Mutex<PollManager<SyncPollFilter>>,
}

impl<C, M> VapFilterClient<C, M> {
	/// Creates new Vap filter client.
...
impl<C, M> Filterable for VapFilterClient<C, M> where
...
impl<T: Filterable + Send + Sync + 'static> VapFilter for T {
