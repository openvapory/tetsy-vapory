...
//! Vap PUB-SUB rpc implementation.
...
use types::{
	...
	ids::BlockId,
	encoded,
	filter::Filter as VapFilter,
};

type Client = Sink<pubsub::Result>;

/// Vap PubSub implementation.
pub struct VapPubSubClient<C> {
	handler: Arc<ChainNotificationHandler<C>>,
	heads_subscribers: Arc<RwLock<Subscribers<Client>>>,
	logs_subscribers: Arc<RwLock<Subscribers<(Client, VapFilter)>>>,
	transactions_subscribers: Arc<RwLock<Subscribers<Client>>>,
	sync_subscribers: Arc<RwLock<Subscribers<Client>>>,
}

impl<C> VapPubSubClient<C>
	where
		C: 'static + Send + Sync
{
	...
	OD: OnDemandRequester + 'static
{
	...
	pub fn light(
		...
		VapPubSubClient::new(Arc::new(fetch), executor, pool_receiver)
	}
}
...
pub trait LightClient: Send + Sync {
	...
	/// Fetch logs.
	fn logs(&self, filter: VapFilter) -> BoxFuture<Vec<Log>>;
}
...
impl<C: Send + Sync + 'static> VapPubSub for VapPubSubClient<C> {
