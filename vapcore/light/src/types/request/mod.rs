...
impl Encodable for Request {
	fn rlp_append(&self, s: &mut RlpStream) {
		...
		// hack around https://github.com/openvapory/tetsy-vapory/issues/4356
...
mod tests {
	...
	fn check_roundtrip<T>(val: T)
		where T: ::tetsy_rlp::Encodable + ::tetsy_rlp::Decodable + PartialEq + ::std::fmt::Debug
	{
		// check as single value.
		let bytes = ::tetsy_rlp::encode(&val);
		let new_val: T = ::tetsy_rlp::decode(&bytes).unwrap();
		assert_eq!(val, new_val);

		// check as list containing single value.
		let list = [val];
		let bytes = ::tetsy_rlp::encode_list(&list);
		let new_list: Vec<T> = ::tetsy_rlp::decode_list(&bytes);
	...
	fn headers_roundtrip() {
		...
		let res = HeadersResponse {
			headers: vec![
				::common_types::encoded::Header::new(::tetsy_rlp::encode(&Header::default()))
			]
		};
	...
	fn responses_vec() {
		...
		let raw = ::tetsy_rlp::encode_list(&reqs);
		assert_eq!(::tetsy_rlp::decode_list::<Response>(&raw), reqs);
	}
