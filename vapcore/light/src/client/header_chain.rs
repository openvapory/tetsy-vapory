...
use common_types::{
	...
	errors::{VapcoreError as Error, BlockError, VapcoreResult},
	header::Header,
	ids::BlockId,
};
...
use tetsy_rlp::{Encodable, Decodable, DecoderError, RlpStream, Rlp};
...
impl HeaderChain {
	...
	pub fn new(
		...
		let genesis = ::tetsy_rlp::encode(&spec.genesis_header());
		let decoded_header = spec.genesis_header();

		let chain = if let Some(current) = db.get(col, CURRENT_KEY)? {
			let curr : BestAndLatest = ::tetsy_rlp::decode(&current).expect("decoding db value failed");
			...
			while let Some(entry) = db.get(col, era_key(cur_number).as_bytes())? {
				let entry: Entry = ::tetsy_rlp::decode(&entry).expect("decoding db value failed");
			...
			if let (&Some(ref hardcoded_sync), HardcodedSync::Allow) = (&spec.hardcoded_sync, allow_hs) {
				...
				for (cht_num, cht_root) in hardcoded_sync.chts.iter().enumerate() {
					batch.put(col, cht_key(cht_num as u64).as_bytes(), &::tetsy_rlp::encode(cht_root));
				}
	...
	fn insert_inner(
		...
		// insert headers and candidates entries and write era to disk.
		{
			...
			transaction.put(self.col, era_key(number).as_bytes(), &::tetsy_rlp::encode(&*cur_era))
		}
		...
		if is_new_best {
			...
			for (&height, entry) in candidates.iter_mut().rev().skip_while(|&(height, _)| *height > number) {
				...
				if height != number {
					let rlp_era = ::tetsy_rlp::encode(&*entry);
			...
			if earliest_era + HISTORY + cht::SIZE <= number {
							...
							if let Some(epoch_transition) = maybe_transition {
								...
								if ancient.hash == era_entry.canonical_hash {
									last_canonical_transition = match self.db.get(self.col, ancient.hash.as_bytes()) {
										...
										Ok(Some(header)) => Some((
											epoch_transition,
											::tetsy_rlp::decode(&header).expect("decoding value from db failed")
										)),
									};
								}
							}
				...
				transaction.put(self.col, cht_key(cht_num).as_bytes(), &::tetsy_rlp::encode(&cht_root));
		...
		// write the best and latest eras to the database.
		{
			...
			transaction.put(self.col, CURRENT_KEY, &::tetsy_rlp::encode(&curr))
		}
		Ok(pending)
	}
			...
			let cht = match self.cht_root(cht_num) {
				...
				None if cht_num != 0 => {
					...
					let entry: Entry = {
						...
						::tetsy_rlp::decode(&bytes).expect("decoding db value failed")
					};
	...
	pub fn cht_root(&self, n: usize) -> Option<H256> {
		match self.db.get(self.col, cht_key(n as u64).as_bytes()) {
			Ok(db_fetch) => db_fetch.map(|bytes| ::tetsy_rlp::decode(&bytes).expect("decoding value from db failed")),
			Err(e) => {
	...
	pub fn insert_pending_transition(&self, batch: &mut DBTransaction, hash: H256, t: &PendingEpochTransition) {
		...
		batch.put(self.col, key.as_bytes(), &*::tetsy_rlp::encode(t));
	}
