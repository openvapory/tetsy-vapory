...
pub enum Error {
	/// RLP decoder error.
	Decoder(::tetsy_rlp::DecoderError),
	/// Empty response.
	Empty,
...
impl BlockReceipts {
	...
	pub fn check_response(&self, cache: &Mutex<::cache::Cache>, receipts: &[Receipt]) -> Result<Vec<Receipt>, Error> {
		...
		let found_root = ::triehash::ordered_trie_root(receipts.iter().map(|r| ::tetsy_rlp::encode(r)));
...
impl Account {
	...
	pub fn check_response(&self, _: &Mutex<::cache::Cache>, proof: &[Bytes]) -> Result<Option<BasicAccount>, Error> {
		...
		let mut db = journaldb::new_tetsy_memory_db();
		...
		match TrieDB::new(&db, &state_root).and_then(|t| t.get(keccak(&self.address).as_bytes()))? {
			Some(val) => {
				Ok(Some(tetsy_rlp::decode::<BasicAccount>(&val)?))
			},
			None => {
...
mod tests {
	...
	use vaptrie::{SecTrieDB, SecTrieDBMut};
	use trie::Recorder;
	use hash::keccak;
	...
	fn check_header_by_hash() {
		...
		let hash = header.hash();
		let raw_header = encoded::Header::new(::tetsy_rlp::encode(&header));
	...
	fn check_header_with_ancestors() {
		...
		let raw_headers = headers.iter()
			.map(|hdr| encoded::Header::new(::tetsy_rlp::encode(hdr)))
			.collect::<Vec<_>>();
		...
		let raw_invalid_successor = encoded::Header::new(::tetsy_rlp::encode(&invalid_successor));
	...
	fn check_body() {
		...
		let req = Body(encoded::Header::new(::tetsy_rlp::encode(&header)).into());
	...
	fn check_receipts() {
		...
		let receipts_root = ::triehash::ordered_trie_root(
			receipts.iter().map(|x| ::tetsy_rlp::encode(x))
		);

		header.set_receipts_root(receipts_root);

		let req = BlockReceipts(encoded::Header::new(::tetsy_rlp::encode(&header)).into());
	...
	fn check_state_proof() {
		...
		let req = Account {
			header: encoded::Header::new(::tetsy_rlp::encode(&header)).into(),
			address: addr,
		};
