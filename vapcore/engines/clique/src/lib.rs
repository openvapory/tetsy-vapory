...
use vapory_types::{Address, H64, H160, H256, U256};
...
impl VoteType {
	...
	pub fn as_rlp(&self) -> Vec<Vec<u8>> {
		match self {
			VoteType::Add => vec![tetsy_rlp::encode(&NULL_MIXHASH), tetsy_rlp::encode(&NONCE_AUTH_VOTE)],
			VoteType::Remove => vec![tetsy_rlp::encode(&NULL_MIXHASH), tetsy_rlp::encode(&NONCE_DROP_VOTE)],
		}
	}
}
...
impl Engine for Clique {
	...
	fn extra_info(&self, header: &Header) -> BTreeMap<String, String> {
		// clique engine seal fields are the same as vapash seal fields
	...
	fn verify_block_basic(&self, header: &Header) -> Result<(), Error> {
		// Largely same as https://github.com/vaporyco/go-vapory/blob/master/consensus/clique/clique.go#L275
	...
	/// see https://github.com/vaporyco/go-vapory/blob/master/consensus/clique/clique.go#L338
	...
	fn populate_from_parent(&self, header: &mut Header, parent: &Header) {
		...
		if header.extra_data().len() < VANITY_LENGTH + SIGNATURE_LENGTH {
			...
			// It's unclear how to prevent creating new blocks unless we are authorized, the best way (and gvap does this too)
