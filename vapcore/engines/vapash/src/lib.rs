...
use tetsy_unexpected::{OutOfBounds, Mismatch};

/// Number of blocks in an vapash snapshot.
// make dependent on difficulty increment divisor?
const SNAPSHOT_BLOCKS: u64 = 5000;
/// Maximum number of blocks allowed in an vapash snapshot.
...
impl From<vapjson::spec::VapashParams> for VapashParams {
	fn from(p: vapjson::spec::VapashParams) -> Self {
		VapashParams {
			...
			block_reward: p.block_reward.map_or_else(
				...
				|reward| {
					match reward {
						vapjson::spec::BlockReward::Single(reward) => {
							let mut ret = BTreeMap::new();
							ret.insert(0, reward.into());
							ret
						},
						vapjson::spec::BlockReward::Multi(multi) => {
							multi.into_iter()
...
/// Engine using Vapash proof-of-work consensus algorithm, suitable for Vapory
...
fn verify_block_unordered(pow: &Arc<VapashManager>, header: &Header) -> Result<(), Error> {
	let seal = VapashSeal::parse_seal(header.seal())?;
	...
	let difficulty = vapash::boundary_to_difficulty(&H256(result.value));
...
// TODO [rphmeier]
//
// for now, this is different than Vapash's own epochs, and signal
// "consensus epochs".
// in this sense, `Vapash` is epochless: the same `EpochVerifier` can be used
// for any block in the chain.
// in the future, we might move the Vapash epoch
...
impl Engine for Vapash {
	...
	fn extra_info(&self, header: &Header) -> BTreeMap<String, String> {
		match VapashSeal::parse_seal(header.seal()) {
			Ok(seal) => map![
	...
	fn on_close_block(&self, block: &mut ExecutedBlock, _parent_header: &Header) -> Result<(), Error> {
		...
		let rewards = match self.vapash_params.block_reward_contract {
			Some(ref c) if number >= self.vapash_params.block_reward_contract_transition => {
			...
			_ => {
				...
				let (_, reward) = self.vapash_params.block_reward.iter()
					.rev()
				...
				let eras_rounds = self.vapash_params.ecip1017_era_rounds;
	...
	fn verify_block_basic(&self, header: &Header) -> Result<(), Error> {
		...
		let min_difficulty = self.vapash_params.minimum_difficulty;
		...
		let difficulty = vapash::boundary_to_difficulty(&H256(quick_get_difficulty(
			&header.bare_hash().0,
			seal.nonce.to_low_u64_be(),
			&seal.mix_hash.0,
			header.number() >= self.vapash_params.progpow_transition
		)));
...
impl Vapash {
	fn calculate_difficulty(&self, header: &Header, parent: &Header) -> U256 {
		...
		let difficulty_hardfork = header.number() >= self.vapash_params.difficulty_hardfork_transition;
		let difficulty_bound_divisor = if difficulty_hardfork {
			self.vapash_params.difficulty_hardfork_bound_divisor
		} else {
			self.vapash_params.difficulty_bound_divisor
		};

		let expip2_hardfork = header.number() >= self.vapash_params.expip2_transition;
		let duration_limit = if expip2_hardfork {
			self.vapash_params.expip2_duration_limit
		} else {
			self.vapash_params.duration_limit
		};

		let frontier_limit = self.vapash_params.homestead_transition;
		...
		let mut target = if header.number() < frontier_limit {
			...
			trace!(target: "vapash", "Calculating difficulty parent.difficulty={}, header.timestamp={}, parent.timestamp={}", parent.difficulty(), header.timestamp(), parent.timestamp());
			//block_diff = parent_diff + parent_diff // 2048 * max(1 - (block_timestamp - parent_timestamp) // 10, -99)
			let (increment_divisor, threshold) = if header.number() < self.vapash_params.eip100b_transition {
				(self.vapash_params.difficulty_increment_divisor, 1)
			} else if parent_has_uncles {
				(self.vapash_params.metropolis_difficulty_increment_divisor, 2)
			} else {
				(self.vapash_params.metropolis_difficulty_increment_divisor, 1)
			};
		...
		if header.number() < self.vapash_params.bomb_defuse_transition {
			if header.number() < self.vapash_params.ecip1010_pause_transition {
				let mut number = header.number();
				let original_number = number;
				for (block, delay) in &self.vapash_params.difficulty_bomb_delays {
			...
			} else if header.number() < self.vapash_params.ecip1010_continue_transition {
				let fixed_difficulty = ((self.vapash_params.ecip1010_pause_transition / EXP_DIFF_PERIOD) - 2) as usize;
				target = cmp::max(min_difficulty, target + (U256::from(1) << fixed_difficulty));
			} else {
				let period = ((parent.number() + 1) / EXP_DIFF_PERIOD) as usize;
				let delay = ((self.vapash_params.ecip1010_continue_transition - self.vapash_params.ecip1010_pause_transition) / EXP_DIFF_PERIOD) as usize;
...
mod tests {
	...
	fn get_default_vapash_params() -> VapashParams {
		VapashParams {
	...
	fn can_do_difficulty_verification_fail() {
		...
		header.set_seal(vec![tetsy_rlp::encode(&H256::zero()), tetsy_rlp::encode(&H64::zero())]);
	...
	fn difficulty_frontier() {
		...
		let difficulty = vapash.calculate_difficulty(&header, &parent_header);
	...
	fn difficulty_classic_bomb_delay() {
		...
		assert_eq!(
			...
			vapash.calculate_difficulty(&header, &parent_header)
		);
