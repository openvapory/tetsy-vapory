...
//! General error types for use in vapcore.

use std::{error, fmt};
use derive_more::{Display, From};
use vapory_types::{U256, U512};
...
pub enum VapcoreError {
	...
	Trie(TrieError),
	/// Error concerning VVM code execution.
	...
	Engine(EngineError),
	/// Vapkey error."
	#[display(fmt = "Vapkey error: {}", _0)]
	Vapkey(VapPublicKeyCryptoError),
	/// RLP decoding errors
...
impl error::Error for VapcoreError {
	fn source(&self) -> Option<&(dyn error::Error + 'static)> {
		use self::VapcoreError::*;
		match self {
			Io(e) => Some(e),
			StdIo(e) => Some(e),
			Trie(e) => Some(e),
...
impl From<&str> for VapcoreError {
	fn from(s: &str) -> Self {
		VapcoreError::Msg(s.into())
	}
}

impl<E> From<Box<E>> for VapcoreError where VapcoreError: From<E> {
	fn from(err: Box<E>) -> VapcoreError {
		VapcoreError::from(*err)
	}
}
