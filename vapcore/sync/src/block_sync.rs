...
use common_types::{
	...
	ids::BlockId,
	errors::{VapcoreError, BlockError, ImportError},
};

const MAX_HEADERS_TO_REQUEST: usize = 128;
const MAX_BODIES_TO_REQUEST_LARGE: usize = 128;
const MAX_BODIES_TO_REQUEST_SMALL: usize = 32; // Size request for tetsy clients prior to 2.4.0
...
impl From<tetsy_rlp::DecoderError> for BlockDownloaderImportError {
	fn from(_: tetsy_rlp::DecoderError) -> BlockDownloaderImportError {
	...
	pub fn collect_blocks(&mut self, io: &mut dyn SyncIo, allow_out_of_order: bool) -> DownloadAction {
		...
		for block_and_receipts in blocks {
			...
			match result {
				...
				Err(VapcoreError::Block(BlockError::UnknownParent(_))) if allow_out_of_order => {
					break;
				},
				Err(VapcoreError::Block(BlockError::UnknownParent(_))) => {
					trace_sync!(self, "Unknown new block parent, restarting sync");
					break;
				},
				Err(VapcoreError::Block(BlockError::TemporarilyInvalid(_))) => {
					debug_sync!(self, "Block temporarily invalid: {:?}, restarting sync", h);
					break;
				},
				Err(VapcoreError::FullQueue(limit)) => {
...
mod tests {
	...
	use tetsy_rlp::{encode_list, RlpStream};
	...
	fn import_bodies() {
		...
		for i in 0..4 {
			...
			let uncles = if i > 0 {
				...
				::tetsy_rlp::EMPTY_LIST_RLP.to_vec()
			};
