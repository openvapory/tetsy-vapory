...
use crate::{
	...
	chain::{
		...
		SyncState, VAP_PROTOCOL_VERSION_62, VAP_PROTOCOL_VERSION_63, MAX_NEW_BLOCK_AGE, MAX_NEW_HASHES,
...
use common_types::{
	...
	ids::BlockId,
	errors::{VapcoreError, ImportError, BlockError},
...
impl SyncHandler {
	...
	pub fn on_peer_new_block(sync: &mut ChainSync, io: &mut dyn SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		match io.chain().import_block(block) {
			...
			Err(VapcoreError::Block(BlockError::UnknownParent(p))) => {
				unknown = true;
	...
	fn on_peer_status(sync: &mut ChainSync, io: &mut dyn SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		if false
			...
			|| (!warp_protocol && (peer.protocol_version < VAP_PROTOCOL_VERSION_62.0 || peer.protocol_version > VAP_PROTOCOL_VERSION_63.0))
		{
