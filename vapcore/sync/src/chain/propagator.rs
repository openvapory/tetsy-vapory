...
mod tests {
	...
	fn should_propagate_service_transaction_to_selected_peers_only() {
		...
		io.peers_info.insert(1, "Gvap".to_owned());
		// and peer#2 is Tetsy, accepting service transactions
		insert_dummy_peer(&mut sync, 2, block_hash);
		io.peers_info.insert(2, "Tetsy-Vapory/v2.6.0/linux/rustc".to_owned());
		// and peer#3 is Tetsy, accepting service transactions
		insert_dummy_peer(&mut sync, 3, block_hash);
		io.peers_info.insert(3, "Tetsy-Vapory/ABCDEFGH/v2.7.3/linux/rustc".to_owned());
	...
	fn should_propagate_service_transaction_is_sent_as_separate_message() {
		...
		// when peer#1 is Tetsy, accepting service transactions
		insert_dummy_peer(&mut sync, 1, block_hash);
		io.peers_info.insert(1, "Tetsy-Vapory/v2.6.0/linux/rustc".to_owned());
