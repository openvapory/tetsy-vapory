...
use tetsy_crypto::publickey::{Signature, recover as ec_recover};
use tetsy_keccak_hash::keccak;
use log::{warn, trace};
use num::{BigUint, Zero, One};
use tetsy_bytes::BytesRef;
use tetsy_crypto::digest;
...
impl TryFrom<vapjson::spec::builtin::Builtin> for Builtin {
	type Error = VapcoreError;

	fn try_from(b: vapjson::spec::builtin::Builtin) -> Result<Self, Self::Error> {
		let native = VaporyBuiltin::from_str(&b.name)?;
...
impl From<vapjson::spec::builtin::Pricing> for Pricing {
	fn from(pricing: vapjson::spec::builtin::Pricing) -> Self {
		match pricing {
			vapjson::spec::builtin::Pricing::Blake2F { gas_per_round } => {
				Pricing::Blake2F(gas_per_round)
			}
			vapjson::spec::builtin::Pricing::Linear(linear) => {
				Pricing::Linear(Linear {
					base: linear.base,
					word: linear.word,
				})
			}
			vapjson::spec::builtin::Pricing::Modexp(exp) => {
			...
			vapjson::spec::builtin::Pricing::AltBn128Pairing(pricer) => {
			...
			vapjson::spec::builtin::Pricing::AltBn128ConstOperations(pricer) => {
...
impl FromStr for VaporyBuiltin {
	type Err = VapcoreError;

	fn from_str(name: &str) -> Result<VaporyBuiltin, Self::Err> {
		match name {
			"identity" => Ok(VaporyBuiltin::Identity(Identity)),
			"ecrecover" => Ok(VaporyBuiltin::EcRecover(EcRecover)),
			"sha256" => Ok(VaporyBuiltin::Sha256(Sha256)),
			"ripemd160" => Ok(VaporyBuiltin::Ripemd160(Ripemd160)),
			"modexp" => Ok(VaporyBuiltin::Modexp(Modexp)),
			"alt_bn128_add" => Ok(VaporyBuiltin::Bn128Add(Bn128Add)),
			"alt_bn128_mul" => Ok(VaporyBuiltin::Bn128Mul(Bn128Mul)),
			"alt_bn128_pairing" => Ok(VaporyBuiltin::Bn128Pairing(Bn128Pairing)),
			"blake2_f" => Ok(VaporyBuiltin::Blake2F(Blake2F)),
			_ => return Err(VapcoreError::Msg(format!("invalid builtin name: {}", name))),
		}
	}
}

impl Implementation for VaporyBuiltin {
	fn execute(&self, input: &[u8], output: &mut BytesRef) -> Result<(), &'static str> {
		match self {
			VaporyBuiltin::Identity(inner) => inner.execute(input, output),
			VaporyBuiltin::EcRecover(inner) => inner.execute(input, output),
			VaporyBuiltin::Sha256(inner) => inner.execute(input, output),
			VaporyBuiltin::Ripemd160(inner) => inner.execute(input, output),
			VaporyBuiltin::Modexp(inner) => inner.execute(input, output),
			VaporyBuiltin::Bn128Add(inner) => inner.execute(input, output),
			VaporyBuiltin::Bn128Mul(inner) => inner.execute(input, output),
			VaporyBuiltin::Bn128Pairing(inner) => inner.execute(input, output),
			VaporyBuiltin::Blake2F(inner) => inner.execute(input, output),
		}
	}
}
...
mod tests {
	...
	use tetsy_bytes::BytesRef;
	use super::{
		BTreeMap, Builtin, VaporyBuiltin, FromStr, Implementation, Linear,
	...
	fn blake2f_cost() {
		let f = Builtin {
			...
			native: VaporyBuiltin::from_str("blake2_f").unwrap(),
		};
		// 5 rounds
	...
	fn blake2f_cost_on_invalid_length() {
		let f = Builtin {
			pricer: map![0 => Pricing::Blake2F(123)],
			native: VaporyBuiltin::from_str("blake2_f").expect("known builtin"),
		};
	...
	fn blake2_f_is_err_on_invalid_length() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 1 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-1
	...
	fn blake2_f_is_err_on_invalid_length_2() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 2 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-2
	...
	fn blake2_f_is_err_on_bad_finalization_flag() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 3 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-3
	...
	fn blake2_f_zero_rounds_is_ok_test_vector_4() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 4 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-4
	...
	fn blake2_f_test_vector_5() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 5 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-5
	...
	fn blake2_f_test_vector_6() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 6 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-6
	...
	fn blake2_f_test_vector_7() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 7 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-7
	...
	fn blake2_f_test_vector_8() {
		let blake2 = VaporyBuiltin::from_str("blake2_f").unwrap();
		// Test vector 8 and expected output from https://github.com/vaporyco/EIPs/blob/master/EIPS/eip-152.md#test-vector-8
	...
	#[test]
	fn identity() {
		let f = VaporyBuiltin::from_str("identity").unwrap();
		let i = [0u8, 1, 2, 3];
	...
	#[test]
	fn sha256() {
		let f = VaporyBuiltin::from_str("sha256").unwrap();
		let i = [0u8; 0];
	...
	#[test]
	fn ripemd160() {
		let f = VaporyBuiltin::from_str("ripemd160").unwrap();
		let i = [0u8; 0];
	...
	#[test]
	fn ecrecover() {
		let f = VaporyBuiltin::from_str("ecrecover").unwrap();
	...
	fn modexp() {
		let f = Builtin {
			...
			native: VaporyBuiltin::from_str("modexp").unwrap(),
		};
	...
	fn bn128_add() {
		...
		let f = Builtin {
			...
			native: VaporyBuiltin::from_str("alt_bn128_add").unwrap(),
		};
	...
	fn bn128_mul() {
		...
		let f = Builtin {
			pricer: map![0 => Pricing::Linear(Linear { base: 0, word: 0 })],
			native: VaporyBuiltin::from_str("alt_bn128_mul").unwrap(),
		};
	...
	fn builtin_pairing() -> Builtin {
		Builtin {
			pricer: map![0 => Pricing::Linear(Linear { base: 0, word: 0 })],
			native: VaporyBuiltin::from_str("alt_bn128_pairing").unwrap(),
		}
	}
	...
	fn from_unknown_linear() {
		let _ = VaporyBuiltin::from_str("foo").unwrap();
	}
	...
	fn is_active() {
		...
		let b = Builtin {
			...
			native: VaporyBuiltin::from_str("identity").unwrap(),
		};
	...
	#[test]
	fn from_json() {
		let b = Builtin::try_from(vapjson::spec::Builtin {
