...
//! Tests of VVM integration with transaction execution.
...
vvm_test!{test_blockhash_eip210: test_blockhash_eip210_int}
fn test_blockhash_eip210(factory: Factory) {
	...
	let mashina = new_eip210_test_mashina();
	...
	for i in 1 .. 257 {
		...
		let mut ex = Executive::new(&mut state, &env_info, &mashina, &schedule);
