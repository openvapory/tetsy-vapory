...
use types::{
	...
	errors::{VapcoreError as Error, VapcoreResult},
...
impl TestBlockChainClient {
		...
		where F: Fn(Header) -> Header
	{
		...
		let txs = match with {
			...
			_ => ::tetsy_rlp::EMPTY_LIST_RLP.to_vec()
		};
	...
	pub fn corrupt_block_parent(&self, n: BlockNumber) {
		...
		let mut rlp = RlpStream::new_list(3);
		rlp.append(&header);
		rlp.append_raw(&::tetsy_rlp::NULL_RLP, 1);
		rlp.append_raw(&::tetsy_rlp::NULL_RLP, 1);
...
pub fn get_temp_state_db() -> StateDB {
	let db = tetsy_kvdb_memorydb::create(NUM_COLUMNS);
...
impl ImportSealedBlock for TestBlockChainClient {
	fn import_sealed_block(&self, _block: SealedBlock) -> VapcoreResult<H256> {
		Ok(H256::zero())
	}
}
...
impl StateInfo for TestState {
	fn nonce(&self, _address: &Address) -> vaptrie::Result<U256> { unimplemented!() }
	fn balance(&self, _address: &Address) -> vaptrie::Result<U256> { unimplemented!() }
	fn storage_at(&self, _address: &Address, _key: &H256) -> vaptrie::Result<H256> { unimplemented!() }
	fn code(&self, _address: &Address) -> vaptrie::Result<Option<Arc<Bytes>>> { unimplemented!() }
}
...
impl IoClient for TestBlockChainClient {
	...
	fn queue_ancient_block(&self, unverified: Unverified, _r: Bytes) -> VapcoreResult<H256> {
