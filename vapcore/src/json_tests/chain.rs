...
pub fn json_chain_test<H: FnMut(&str, HookType)>(path: &Path, json_data: &[u8], start_stop_hook: &mut H) -> Vec<String> {
	...
	for (name, blockchain) in tests.into_iter() {
		...
		let mut fail = false;
		{
			...
			let spec = {
				let mut spec = match VvmTestClient::fork_spec_from_json(&blockchain.network) {
					Some(spec) => spec,
					None => {
...

			{
				...
				if vapjson::test_helpers::blockchain::Engine::NoProof == blockchain.engine {
...
mod block_tests {
	...
	//Issue: https://github.com/openvapory/tetsy-vapory/issues/11085
	...
	//  Ignored in currents.json, issue: https://github.com/openvapory/tetsy-vapory/issues/11073
