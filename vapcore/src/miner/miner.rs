...
use vapcore_miner::gas_pricer::GasPricer;
use vapcore_miner::local_accounts::LocalAccounts;
use vapcore_miner::pool::{self, TransactionQueue, VerifiedTransaction, QueueStatus, PrioritizationStrategy, TxStatus};
use vapcore_miner::service_transaction_checker::ServiceTransactionChecker;
...
use types::{
	...
	errors::{VapcoreError as Error, ExecutionError},
	receipt::RichReceipt,
	transaction::{
		self,
		Action,
...
impl Miner {
	...
	// TODO: (https://github.com/openvapory/tetsy-vapory/issues/10407)
			...
			C: BlockChain + CallContract + BlockProducer + SealedBlockImporter + Nonce + Sync,
	{
		...
		//  2. or someone is requesting `vap_getWork`
		// When either condition is true, `sealing.enabled` is flipped to true (and if you
		// have no more local transactions or stop calling `vap_getWork`, it is set to `false`).
