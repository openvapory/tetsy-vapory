...
pub struct Interpreter<Cost: CostType> {
	...
	informant: informant::VvmInformant,
	do_trace: bool,
	done: bool,
...
impl<Cost: CostType> Interpreter<Cost> {
	...
	pub fn new(mut params: ActionParams, cache: Arc<SharedCache>, schedule: &Schedule, depth: usize) -> Interpreter<Cost> {
		...
		let informant = informant::VvmInformant::new(depth);
	...
	fn step_inner(&mut self, ext: &mut dyn tetsy_vm::Ext) -> InterpreterResult {
		let result = match self.resume_result.take() {
			...
			None => {
				...
				vvm_debug!({ self.informant.before_instruction(self.reader.position, instruction, info, &self.gasometer.as_mut().expect(GASOMETER_PROOF).current_gas, &self.stack) });
				...
				vvm_debug!({ self.informant.after_instruction(instruction) });
				result
			},
		};
