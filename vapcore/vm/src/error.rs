...
impl From<Box<vaptrie::TrieError>> for Error {
	fn from(err: Box<vaptrie::TrieError>) -> Self {
		Error::Internal(format!("Internal error: {}", err))
	}
}

impl From<vaptrie::TrieError> for Error {
	fn from(err: vaptrie::TrieError) -> Self {
		Error::Internal(format!("Internal error: {}", err))
	}
}
