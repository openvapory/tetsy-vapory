...
use vapkey::Password;
use tetsy_crypto::publickey::{Public, Signature, Error};
use log::warn;
...
impl EngineSigner for (Arc<AccountProvider>, Address, Password) {
	fn sign(&self, hash: H256) -> Result<Signature, Error> {
		match self.0.sign(self.1, Some(self.2.clone()), hash) {
			...
			Err(SignError::SStore(accounts::Error::VapCrypto(err))) => Err(Error::Custom(err.to_string())),
			Err(SignError::SStore(accounts::Error::VapPublicKeyCrypto(err))) => {
