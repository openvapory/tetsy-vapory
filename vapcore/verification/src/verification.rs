...
/// Phase 2 verification. Perform costly checks such as transaction signatures and block nonce for vapash.
...
mod tests {
	...
	fn test_verify_block_basic_with_invalid_transactions() {
		...
		let block = {
			let mut rlp = tetsy_rlp::RlpStream::new_list(3);
	...
	fn test_verify_block() {
		...
		let good_transactions_root = ordered_trie_root(good_transactions.iter().map(|t| ::tetsy_rlp::encode::<UnverifiedTransaction>(t)));
		let eip86_transactions_root = ordered_trie_root(eip86_transactions.iter().map(|t| ::tetsy_rlp::encode::<UnverifiedTransaction>(t)));
		...
		match basic_test(&create_test_block_with_data(&bad_header, &eip86_transactions, &good_uncles), engine) {
			Err(Error::Transaction(ref e)) if e == &tetsy_crypto::publickey::Error::InvalidSignature.into() => (),
