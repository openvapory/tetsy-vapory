...
use tetsy_keccak_hash::{KECCAK_EMPTY, KECCAK_NULL_RLP, keccak};
...
fn encoding_basic() {
	...
	let thin_rlp = ::tetsy_rlp::encode(&account);
	assert_eq!(::tetsy_rlp::decode::<BasicAccount>(&thin_rlp).unwrap(), account);
...
fn encoding_empty_acc() {
	let mut db = get_temp_state_db();
	assert_eq!(from_fat_rlp(&mut AccountDBMut::from_hash(db.as_hash_db_mut(), keccak(Address::zero())), Rlp::new(&::tetsy_rlp::NULL_RLP), H256::zero()).unwrap(), (ACC_EMPTY, None));
}
