...
mod test {
	...
	fn make_vault_dir_path_succeeds() {
		...
		assert_eq!(&make_vault_dir_path("/home/user/tetsy", "vault", true).unwrap(), &Path::new("/home/user/tetsy/vault"));
		assert_eq!(&make_vault_dir_path("/home/user/tetsy", "*bad-name*", false).unwrap(), &Path::new("/home/user/tetsy/*bad-name*"));
	}

	#[test]
	fn make_vault_dir_path_fails() {
		assert!(make_vault_dir_path("/home/user/tetsy", "*bad-name*", true).is_err());
	}
