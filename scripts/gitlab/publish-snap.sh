...
SNAP_PACKAGE="tetsy_"$VERSION"_"$BUILD_ARCH".snap"
# Choose snap release channel based on tetsy vapory version track
case ${TRACK} in
...
echo $SNAPCRAFT_LOGIN_TETSY_BASE64 | base64 --decode > snapcraft.login
snapcraft login --with snapcraft.login
snapcraft push --release $CHANNEL $SNAP_PACKAGE
snapcraft status tetsy
snapcraft logout
