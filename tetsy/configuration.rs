...
use tetsy_version::{version_data, version};
use bytes::Bytes;
...
use tetsy_rpc::NetworkSettings;
use cache::CacheConfig;
use helpers::{to_duration, to_mode, to_block_id, to_u256, to_pending_set, to_price, gvap_ipc_path, tetsy_ipc_path, to_bootnodes, to_addresses, to_address, to_queue_strategy, to_queue_penalization};
...
use account::{AccountCmd, NewAccount, ListAccounts, ImportAccounts, ImportFromGvapAccounts};
use snapshot_cmd::{self, SnapshotCommand};
use network::{IpFilter, NatType};

const DEFAULT_MAX_PEERS: u16 = 50;
const DEFAULT_MIN_PEERS: u16 = 25;
pub const VAPORSCAN_VAP_PRICE_ENDPOINT: &str = "https://api.vaporscan.io/api?module=stats&action=vapprice";
...
impl Configuration {
	...
	///
	/// ```
	/// let _cfg = tetsy_vapory::Configuration::parse_cli(&["--light", "--chain", "kovan"]).unwrap();
	/// ```
	...
	pub(crate) fn into_command(self) -> Result<Execute, String> {
		...
		let gvap_compatibility = self.args.flag_gvap;
		...
		} else if self.args.flag_import_gvap_keys {
				let account_cmd = AccountCmd::ImportFromGvap(
				ImportFromGvapAccounts {
					spec: spec,
					to: dirs.keys,
	...
	fn author(&self) -> Result<Address, String> {
		to_address(self.args.arg_vaporbase.clone().or(self.args.arg_author.clone()))
	}
	...
	fn gas_pricer_config(&self) -> Result<GasPricerConfig, String> {
		fn wei_per_gas(usd_per_tx: f32, usd_per_vap: f32) -> U256 {
		...
		if "auto" == self.args.arg_usd_per_vap {
			Ok(GasPricerConfig::Calibrated {
				usd_per_tx: usd_per_tx,
				recalibration_period: to_duration(self.args.arg_price_update_period.as_str())?,
				api_endpoint: VAPORSCAN_VAP_PRICE_ENDPOINT.to_string(),
			})
		} else if let Ok(usd_per_vap_parsed) = to_price(&self.args.arg_usd_per_vap) {
			let wei_per_gas = wei_per_gas(usd_per_tx, usd_per_vap_parsed);

			info!(
				"Using a fixed conversion rate of Ξ1 = {} ({} wei/gas)",
				Colour::White.bold().paint(format!("US${:.2}", usd_per_vap_parsed)),
			...
			Ok(GasPricerConfig::Calibrated {
				usd_per_tx: usd_per_tx,
				recalibration_period: to_duration(self.args.arg_price_update_period.as_str())?,
				api_endpoint: self.args.arg_usd_per_vap.clone(),
			})
		}
	}
	...
	fn ipc_path(&self) -> String {
		if self.args.flag_gvap {
			gvap_ipc_path(self.args.flag_testnet)
		} else {
			tetsy_ipc_path(
...
mod tests {
	...
	use tempdir::TempDir;
	use vapcore::miner::MinerOptions;
	...
	fn test_command_account_new() {
		let args = vec!["tetsy", "account", "new"];
		let conf = parse(&args);
	...
	fn test_command_account_list() {
		let args = vec!["tetsy", "account", "list"];
		let conf = parse(&args);
	...
	fn test_command_account_import() {
		let args = vec!["tetsy", "account", "import", "my_dir", "another_dir"];
		let conf = parse(&args);
	...
	fn test_command_wallet_import() {
		let args = vec!["tetsy", "wallet", "import", "my_wallet.json", "--password", "pwd"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_import() {
		let args = vec!["tetsy", "import", "blockchain.json"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_export() {
		let args = vec!["tetsy", "export", "blocks", "blockchain.json"];
		let conf = parse(&args);
	...
	fn test_command_state_export() {
		let args = vec!["tetsy", "export", "state", "state.json"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_export_with_custom_format() {
		let args = vec!["tetsy", "export", "blocks", "--format", "hex", "blockchain.json"];
		let conf = parse(&args);
		assert_eq!(conf.into_command().unwrap().cmd, Cmd::Blockchain(BlockchainCmd::Export(ExportBlockchain {
			spec: Default::default(),
			cache_config: Default::default(),
	...
	fn test_command_signer_new_token() {
		let args = vec!["tetsy", "signer", "new-token"];
		let conf = parse(&args);
		...
		assert_eq!(conf.into_command().unwrap().cmd, Cmd::SignerToken(WsConfiguration {
			...
			origins: Some(vec!["tetsy://*".into(),"chrome-extension://*".into(), "moz-extension://*".into()]),
			hosts: Some(vec![]),
	...
	fn test_ws_max_connections() {
		let args = vec!["tetsy", "--ws-max-connections", "1"];
		let conf = parse(&args);
	...
	fn should_parse_mining_options() {
		...
		// when
		let conf0 = parse(&["tetsy"]);
		let conf2 = parse(&["tetsy", "--tx-queue-strategy", "gas_price"]);
	...
	fn should_fail_on_force_reseal_and_reseal_min_period() {
		let conf = parse(&["tetsy", "--chain", "dev", "--force-sealing", "--reseal-min-period", "0"]);
	...
	fn should_parse_updater_options() {
		// when
		let conf0 = parse(&["tetsy", "--auto-update", "all", "--no-consensus", "--auto-update-delay", "300"]);
		let conf1 = parse(&["tetsy", "--auto-update=xxx"]);
	...
	fn should_parse_network_settings() {
		...
		// when
		let conf = parse(&["tetsy", "--testnet", "--identity", "testname"]);
	...
	#[test]
	fn should_parse_rpc_settings_with_gvap_compatiblity() {
		// given
		fn assert(conf: Configuration) {
			...
			assert_eq!(conf.rpc_apis(), "web3,vap".to_owned());
		}

		// when
		let conf1 = parse(&["tetsy", "-j",
						 "--jsonrpc-port", "8000",
						 "--jsonrpc-interface", "all",
						 "--jsonrpc-cors", "*",
						 "--jsonrpc-apis", "web3,vap"
						 ]);
		let conf2 = parse(&["tetsy", "--rpc",
							"--rpcport", "8000",
							"--rpcaddr", "all",
							"--rpccorsdomain", "*",
							"--rpcapi", "web3,vap"
	...
	fn should_parse_rpc_hosts() {
		...
		// when
		let conf0 = parse(&["tetsy"]);
		let conf1 = parse(&["tetsy", "--jsonrpc-hosts", "none"]);
		let conf2 = parse(&["tetsy", "--jsonrpc-hosts", "all"]);
		let conf3 = parse(&["tetsy", "--jsonrpc-hosts", "tetcoin.org,something.io"]);
	...
	fn should_parse_ipfs_hosts() {
		...
		// when
		let conf0 = parse(&["tetsy"]);
		let conf1 = parse(&["tetsy", "--ipfs-api-hosts", "none"]);
		let conf2 = parse(&["tetsy", "--ipfs-api-hosts", "all"]);
		let conf3 = parse(&["tetsy", "--ipfs-api-hosts", "tetcoin.org,something.io"]);
	...
	fn should_parse_ipfs_cors() {
		...
		// when
		let conf0 = parse(&["tetsy"]);
		let conf1 = parse(&["tetsy", "--ipfs-api-cors", "*"]);
		let conf2 = parse(&["tetsy", "--ipfs-api-cors", "http://tetcoin.org,http://something.io"]);
	...
	fn should_parse_ui_configuration() {
		...
		// when
		let conf0 = parse(&["tetsy", "--ui-path=signer"]);
		let conf1 = parse(&["tetsy", "--ui-path=signer", "--ui-no-validation"]);
		let conf2 = parse(&["tetsy", "--ui-path=signer", "--ui-port", "3123"]);
		let conf3 = parse(&["tetsy", "--ui-path=signer", "--ui-interface", "test"]);
		let conf4 = parse(&["tetsy", "--ui-path=signer", "--force-ui"]);

		// then
		assert_eq!(conf0.directories().signer, "signer".to_owned());

		assert!(conf1.ws_config().unwrap().hosts.is_some());
		assert_eq!(conf1.ws_config().unwrap().origins, Some(vec!["tetsy://*".into(), "chrome-extension://*".into(), "moz-extension://*".into()]));
	...
	fn should_not_bail_on_empty_line_in_reserved_peers() {
		...
		let args = vec!["tetsy", "--reserved-peers", filename.to_str().unwrap()];
	...
	fn test_mining_preset() {
		let args = vec!["tetsy", "--config", "mining"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_non_standard_ports_preset() {
		let args = vec!["tetsy", "--config", "non-standard-ports"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_insecure_preset() {
		let args = vec!["tetsy", "--config", "insecure"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				...
				// "web3,vap,net,personal,tetsy,tetsy_set,traces,rpc,tetsy_accounts");
	...
	fn test_dev_insecure_preset() {
		let args = vec!["tetsy", "--config", "dev-insecure"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				assert_eq!(c.net_settings.chain, "dev");
	...
	fn test_override_preset() {
		let args = vec!["tetsy", "--config", "mining", "--min-peers=99"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_identity_arg() {
		let args = vec!["tetsy", "--identity", "Somebody"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_apply_ports_shift() {
		...
		// when
		let conf0 = parse(&["tetsy", "--ports-shift", "1", "--stratum"]);
		let conf1 = parse(&["tetsy", "--ports-shift", "1", "--jsonrpc-port", "8544"]);
	...
	fn should_resolve_external_nat_hosts() {
		// Ip works
		let conf = parse(&["tetsy", "--nat", "extip:1.1.1.1"]);
		...
		let conf = parse(&["tetsy", "--nat", "extip:192.168.1.1:123"]);
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().ip().to_string(), "192.168.1.1");
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().port(), 30303);

		// Hostname works
		let conf = parse(&["tetsy", "--nat", "extip:vapory.org"]);
		assert!(conf.net_addresses().unwrap().1.is_some());
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().port(), 30303);

		// Hostname works, garbage at the end is discarded
		let conf = parse(&["tetsy", "--nat", "extip:vapory.org:whatever bla bla 123"]);
		assert!(conf.net_addresses().unwrap().1.is_some());
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().port(), 30303);

		// Garbage is error
		let conf = parse(&["tetsy", "--nat", "extip:blabla"]);
	...
	fn should_expose_all_servers() {
		...
		// when
		let conf0 = parse(&["tetsy", "--unsafe-expose"]);
	...
	#[test]
	fn allow_ips() {
		let all = parse(&["tetsy", "--allow-ips", "all"]);
		let private = parse(&["tetsy", "--allow-ips", "private"]);
		let block_custom = parse(&["tetsy", "--allow-ips", "-10.0.0.0/8"]);
		let combo = parse(&["tetsy", "--allow-ips", "public 10.0.0.0/8 -1.0.0.0/8"]);
		let ipv6_custom_public = parse(&["tetsy", "--allow-ips", "public fc00::/7"]);
		let ipv6_custom_private = parse(&["tetsy", "--allow-ips", "private -fc00::/7"]);
	...
	fn should_use_correct_cache_path_if_base_is_set() {
		...
		let std = parse(&["tetsy"]);
		let base = parse(&["tetsy", "--base-path", "/test"]);
	...
	fn should_respect_only_max_peers_and_default() {
		let args = vec!["tetsy", "--max-peers=50"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_respect_only_max_peers_less_than_default() {
		let args = vec!["tetsy", "--max-peers=5"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_respect_only_min_peers_and_default() {
		let args = vec!["tetsy", "--min-peers=5"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				assert_eq!(c.net_conf.min_peers, 5);
	...
	fn should_respect_only_min_peers_and_greater_than_default() {
		let args = vec!["tetsy", "--min-peers=500"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_parse_secretstore_cors() {
		...
		// when
		let conf0 = parse(&["tetsy"]);
		let conf1 = parse(&["tetsy", "--secretstore-http-cors", "*"]);
		let conf2 = parse(&["tetsy", "--secretstore-http-cors", "http://tetcoin.org,http://something.io"]);
