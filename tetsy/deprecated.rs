...
pub fn find_deprecated(args: &Args) -> Vec<Deprecated> {
	...
	if args.arg_vaporbase.is_some() {
		result.push(Deprecated::Replaced("--vaporbase", "--author"));
	}
...
mod tests {
	...
	fn test_find_deprecated() {
		...
		assert_eq!(find_deprecated(&{
			...
			args.arg_vaporbase = Some(Default::default());
			...
			Deprecated::Replaced("--vaporbase", "--author"),
