...
mod accounts {
	...
	impl ::vapcore::miner::LocalAccounts for AccountProvider {
	...
	pub fn prepare_account_provider(_spec: &SpecType, _dirs: &Directories, _data_dir: &str, _cfg: AccountsConfig, _passwords: &[Password]) -> Result<AccountProvider, String> {
		warn!("Note: Your instance of Tetsy Vapory is running without account support. Some CLI options are ignored.");
		Ok(AccountProvider)
	}
	...
	pub fn miner_author(_spec: &SpecType, _dirs: &Directories, _account_provider: &Arc<AccountProvider>, _engine_signer: Address, _passwords: &[Password]) -> Result<Option<::vapcore::miner::Author>, String> {
		Ok(None)
	}
...
mod accounts {
	...
	pub fn prepare_account_provider(spec: &SpecType, dirs: &Directories, data_dir: &str, cfg: AccountsConfig, passwords: &[Password]) -> Result<AccountProvider, String> {
		...
		let vapstore = VapStore::open_with_iterations(dir, cfg.iterations).map_err(|e| format!("Could not open keys directory: {}", e))?;
		if cfg.refresh_time > 0 {
			vapstore.set_refresh_time(::std::time::Duration::from_secs(cfg.refresh_time));
		}
	...
	impl ::vapcore::miner::LocalAccounts for LocalAccounts {
	...
	pub fn miner_author(spec: &SpecType, dirs: &Directories, account_provider: &Arc<AccountProvider>, engine_signer: Address, passwords: &[Password]) -> Result<Option<::vapcore::miner::Author>, String> {
		...
		for password in passwords {
			let signer = tetsy_rpc::signer::EngineSigner::new(
				account_provider.clone(),
				engine_signer,
				password.clone(),
			);
			if signer.sign(Default::default()).is_ok() {
				author = Some(::vapcore::miner::Author::Sealer(Box::new(signer)));
			}
		}
		if author.is_none() {
	...
	fn insert_dev_account(account_provider: &AccountProvider) {
		let secret = tetsy_crypto::publickey::Secret::from_str("4d5db4107d237df6a3d58ee5f70ae63d73d7658d4026f2eefd2f204c81682cb7".into()).expect("Valid account;qed");
		let dev_account = tetsy_crypto::publickey::KeyPair::from_secret(secret.clone()).expect("Valid secret produces valid key;qed");
