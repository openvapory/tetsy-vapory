...
use tetsy_version::version_data;
...
impl str::FromStr for SpecType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let spec = match s {
			"vap" | "vapory"  | "foundation" | "mainnet" => SpecType::Foundation,
...
impl Default for GasPricerConfig {
	fn default() -> Self {
		GasPricerConfig::Calibrated {
			...
			api_endpoint: configuration::VAPORSCAN_VAP_PRICE_ENDPOINT.to_string(),
		}
	}
}
...
mod tests {
	...
	fn test_spec_type_parsing() {
		assert_eq!(SpecType::Foundation, "vap".parse().unwrap());
		assert_eq!(SpecType::Foundation, "vapory".parse().unwrap());
		...
		assert_eq!(SpecType::VaporCore, "vaporcore".parse().unwrap());
	...
	fn test_spec_type_display() {
		...
		assert_eq!(format!("{}", SpecType::VaporCore), "vaporcore");
