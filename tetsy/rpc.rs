...
pub use tetsy_rpc::{IpcServer, HttpServer, RequestMiddleware};
pub use tetsy_rpc::ws::{Server as WsServer, ws};
	...
	fn default() -> Self {
		IpcConfiguration {
			...
			socket_addr: if cfg!(windows) {
				...
				tetsy_ipc_path(&data_dir, "$BASE/jsonrpc.ipc", 0)
			},
			chmod: "660".into(),
...
pub fn new_ws<D: rpc_apis::Dependencies>(
	...
	match start_result {
		...
		Err(rpc::ws::Error::WsError(ws::Error {
			...
			format!("WebSockets address {} is already in use, make sure that another instance of an Vapory client is not running or change the address using the --ws-port and --ws-interface options.", url)
		),
...
pub fn new_http<D: rpc_apis::Dependencies>(
	...
	match start_result {
		...
		Err(ref err) if err.kind() == io::ErrorKind::AddrInUse => Err(
			format!("{} address {} is already in use, make sure that another instance of an Vapory client is not running or change the address using the --{}-port and --{}-interface options.", id, url, options, options)
		),
