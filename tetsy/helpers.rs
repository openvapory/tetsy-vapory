...
use vapcore::client::{DatabaseCompactionProfile, ClientConfig};
use vapcore::miner::{PendingSet, Penalization};
...
// TODO: should we bring it back to vapory-types?
...
/// Returns default gvap ipc path.
pub fn gvap_ipc_path(testnet: bool) -> String {
	// Windows path should not be hardcoded here.
	// Instead it should be a part of path::vapory
	if cfg!(windows) {
		return r"\\.\pipe\gvap.ipc".to_owned();
	}

	if testnet {
		path::vapory::with_testnet("gvap.ipc").to_str().unwrap().to_owned()
	} else {
		path::vapory::with_default("gvap.ipc").to_str().unwrap().to_owned()
	}
}
...
pub fn default_network_config() -> ::sync::NetworkConfiguration {
	...
	NetworkConfiguration {
		...
		client_version: ::tetsy_version::version(),
	}
}
...
mod tests {
	...
	use vapcore::miner::PendingSet;
	use vapkey::Password;
	use types::{
		ids::BlockId,
		client_types::Mode,
	};
	use super::{to_duration, to_mode, to_block_id, to_u256, to_pending_set, to_address, to_addresses, to_price, gvap_ipc_path, to_bootnodes, join_set, password_from_file};
...
"#).unwrap();
	...
	#[test]
	#[cfg(windows)]
	fn test_gvap_ipc_path() {
		assert_eq!(gvap_ipc_path(true), r"\\.\pipe\gvap.ipc".to_owned());
		assert_eq!(gvap_ipc_path(false), r"\\.\pipe\gvap.ipc".to_owned());
	}
	...
	fn test_gvap_ipc_path() {
		use path;
		assert_eq!(gvap_ipc_path(true), path::vapory::with_testnet("gvap.ipc").to_str().unwrap().to_owned());
		assert_eq!(gvap_ipc_path(false), path::vapory::with_default("gvap.ipc").to_str().unwrap().to_owned());
	}
