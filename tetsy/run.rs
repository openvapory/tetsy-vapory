	...
	where Cr: Fn(String) + 'static + Send
{
	...
	//print out running tetsy environment
	...
	let mut config = light_client::Config {
		...
		chain_column: ::vapcore_db::COL_LIGHT_CHAIN,
		verify_full: true,
	...
	*sync_handle.write() = Arc::downgrade(&light_sync);
	...
	// fetch service
	let fetch = tetsy_fetch::Client::new(FETCH_LIGHT_NUM_DNS_THREADS).map_err(|e| format!("Error starting fetch client: {:?}", e))?;
	...
	let deps_for_rpc_apis = Arc::new(rpc_apis::LightDependencies {
		...
		fetch,
		gvap_compatibility: cmd.gvap_compatibility,
		...
		Rr: Fn() + 'static + Send
{
	...
	// fetch service
	let fetch = tetsy_fetch::Client::new(FETCH_FULL_NUM_DNS_THREADS).map_err(|e| format!("Error starting fetch client: {:?}", e))?;
	...
	let store = {
		...
		let store = ::local_store::create(db.key_value().clone(), ::vapcore_db::COL_NODE_INFO, node_info);
