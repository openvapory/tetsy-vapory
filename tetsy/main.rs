...
use vapcore_logger::setup_log;
use tetsy_vapory::{start, ExecutionAction};
use tetsy_daemonize::AsHandle;
use parking_lot::{Condvar, Mutex};

const PLEASE_RESTART_EXIT_CODE: i32 = 69;
const TETSY_EXECUTABLE_NAME: &str = "tetsy";
...
fn main_direct(force_can_restart: bool) -> i32 {
	...
	let mut conf = {
		...
		tetsy_vapory::Configuration::parse_cli(&args).unwrap_or_else(|e| e.exit())
	};
	...
	// see: `https://github.com/tetcoin/tetsy-daemonize/pull/13` for more info
	let handle = if let Some(pid) = conf.args.arg_daemon_pid_file.clone() {
		...
		match tetsy_daemonize::daemonize(pid) {
			Ok(h) => Some(h),
			Err(e) => {
				error!(
					"{}",
...
fn main() {
	...
	// the user has specified to run its originally installed binary (not via `tetsy-updater`)
	...
	let same_name = exe_path
		.as_ref()
		.map_or(false, |p| {
			p.file_stem().map_or(false, |n| n == TETSY_EXECUTABLE_NAME)
		});
