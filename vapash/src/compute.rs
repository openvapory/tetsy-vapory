...
const MIX_WORDS: usize = VAPASH_MIX_BYTES / 4;
...
pub fn slow_hash_block_number(block_number: u64) -> H256 {
	SeedHashCompute::resume_compute_seedhash([0u8; 32], 0, block_number / VAPASH_EPOCH_LENGTH)
}
...
fn hash_compute(light: &Light, full_size: usize, header_hash: &H256, nonce: u64) -> ProofOfWork {
...

	{
		...
		{
			compile_error!("tetsy-vapory currently only supports little-endian targets");
		}
...
pub fn calculate_dag_item(node_index: u32, cache: &[Node]) -> Node {
	...
	debug_assert_eq!(NODE_WORDS, 16);
	for i in 0..VAPASH_DATASET_PARENTS as u32 {
...
mod test {
	...
	fn test_get_cache_size() {
		// https://github.com/vaporyco/wiki/wiki/Vapash/ef6b93f9596746a088ea95d01ca2778be43ae68f#data-sizes
		assert_eq!(16776896usize, get_cache_size(0));
		assert_eq!(16776896usize, get_cache_size(1));
		assert_eq!(16776896usize, get_cache_size(VAPASH_EPOCH_LENGTH - 1));
		assert_eq!(16907456usize, get_cache_size(VAPASH_EPOCH_LENGTH));
		assert_eq!(16907456usize, get_cache_size(VAPASH_EPOCH_LENGTH + 1));
		assert_eq!(284950208usize, get_cache_size(2046 * VAPASH_EPOCH_LENGTH));
		assert_eq!(285081536usize, get_cache_size(2047 * VAPASH_EPOCH_LENGTH));
		assert_eq!(285081536usize, get_cache_size(2048 * VAPASH_EPOCH_LENGTH - 1));
	}
	...
	fn test_get_data_size() {
		...
		assert_eq!(1073739904usize, get_data_size(VAPASH_EPOCH_LENGTH - 1));
		assert_eq!(1082130304usize, get_data_size(VAPASH_EPOCH_LENGTH));
		assert_eq!(1082130304usize, get_data_size(VAPASH_EPOCH_LENGTH + 1));
		assert_eq!(18236833408usize, get_data_size(2046 * VAPASH_EPOCH_LENGTH));
		assert_eq!(18245220736usize, get_data_size(2047 * VAPASH_EPOCH_LENGTH));
	}
	...
	fn test_drop_old_data() {
		...
		let second = builder.light(tempdir.path(), VAPASH_EPOCH_LENGTH).to_file().unwrap().to_owned();
		assert!(fs::metadata(&first).is_ok());

		let _ = builder.light(tempdir.path(), VAPASH_EPOCH_LENGTH * 2).to_file();
		assert!(fs::metadata(&first).is_err());
		assert!(fs::metadata(&second).is_ok());

		let _ = builder.light(tempdir.path(), VAPASH_EPOCH_LENGTH * 3).to_file();
